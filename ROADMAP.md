...
- 芭蕾页顶部标题左侧新增紧凑 `week N` 入口；点击后在弹窗中预览固定粉色手作底图，并可复制图片或下载 `1280×1710` PNG。
- `2026-07-27` 至 `2026-08-02` 固定为 `week 2`，北京时间每周一自动加一；浏览器只把版本化底图与透明手绘数字本地拼合，不每周调用 AI，不上传图片，也不依赖服务器定时任务。
- 同周且模板版本未变化时只复用当前页面缓存；进入新周会丢弃旧内存结果。Owner 后续只需放入新的无数字底图并更新 `dash/assets/ballet-week-cover/template.json` 的文件名和版本，页面脚本无需改动。
