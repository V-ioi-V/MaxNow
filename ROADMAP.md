...
- 生产每日 00:17 抓取当前保持“待启用”：固定 20 分钟的 `PHPSESSID` 活跃寿命实验正在运行，新增生产请求会污染实验频率和滑动窗口判断。实验结束或 Owner 明确接受污染后，才能启用 timer 并执行首次生产同步。
- 服务器已在内部密封 host-bound systemd 加密凭据，并完成每日 00:17 rolling / 每月 1 日 00:47 full 的 unit 模板；`LoadCredentialEncrypted`、非敏感凭据版本和 enable gate 共同控制运行。当前 timer / gate 仍关闭；`PHPSESSID`、Cookie 和原始响应不得进入 Git、日志、前端、聊天、环境变量或命令参数。
- 身份失效时立即停止网络重试、保留旧缓存，并展示安全错误与“请在电脑微信重新登录并刷新凭据”；只有凭据版本变化后才允许再次尝试。
- 当前 20 分钟实验只能证明持续活动下的 Session 寿命，不能证明空闲 Session 寿命，也不能据此承诺长期自动续期。
...
- 阶段 0A-1B｜只读数据与页面：已在实现分支完成；生产抓取仍待实验结束后启用和验证，不能把样例缓存当作每日同步已运行。
