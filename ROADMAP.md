...
- 阶段 0A-1B｜只读数据与页面：已部署到 MaxNow；每天 09:00 / 12:00 / 15:00 / 18:00 / 22:00 rolling、周日额外 14:30 课表刷新、周日 20:00 周简报截止刷新与每月 1 日 00:47 full timer 已启用，刷新预约、候补、上课记录、课程卡和周课表。登录失效后停止请求并保留旧数据。
...
- 周简报使用最近一个已经到达的北京时间周日 20:00 作为统计截止点；rolling timer 在周日 20:00 增加一次既有只读同步，不保存周简报成品，也不进入 14:20 自动抢课关键路径。
