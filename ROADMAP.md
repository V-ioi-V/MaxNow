...
- 建议先使用 `feature/ballet-module` 建立学习模块；真实约课能力另用 `feature/ballet-booking`，避免页面数据与写操作一起上线。
- 产品目标不是单纯抢课，而是让 Owner 看清下一节课、本周计划、未来预约、实际上课记录、学习进度和近期重点；自动约课只作为最后一层受控能力。
- 已确认页面入口位于 Home 下、豆奶上，名称“芭蕾”、副标题“课程与进度”；独立页使用粉玫瑰 + 白卡语义，Home 只放紧凑摘要。当前继续留在 Later，不挤占可靠性与 Blog 主线，也不在规划分支提前修改导航代码。
...
- 阶段 0A｜私有账本：用脱敏样本确认课程 / 场次 / 预约 / 上课记录的源 ID 和稳定兜底键；建立服务器本地 `0600` canonical ledger，首次全量回填，日常重扫最近 60 个逻辑日并 upsert，每月 1 日 00:47 全量校验。写入必须 schema 校验、唯一性校验并原子替换，重复同步不改变历史总数；改名 / 换老师只能通过唯一场次 alias 合并，歧义时停止。
- 阶段 0B｜同步状态与预约快照：以北京时间 00:00 为逻辑日界、00:17 实际运行并结算前一日；`sync-state.json` 单独记录最近尝试、最近成功、数据变更时间、抓取窗口、记录数、连续失败和安全错误码。`attendance-ledger.json` 只在成功时替换，`booking-snapshot.json` 每日同批刷新且默认 TTL 36 小时；身份失效不重试，连续 3 次失败升级 Cloud 异常。
- 阶段 0C｜read model：定义脱敏 `ballet.*`，预计算全局、月度、年度的节数 / 分钟；课程同时按 `courseType`（芭蕾、软开、肌肉素质、技术技巧、其他）和 `level`（L1、L1.5、L2、L3、L4、无级别）聚合，归类规则版本化，`L1.5` 先于 `L1` 匹配。确认 personal-wiki 中目标与笔记的唯一来源格式，不把 Markdown 变成第二份上课事实源。
- 阶段 1｜只读 MVP：实现独立页面“芭蕾 / 课程与进度”，展示下一节课、本周训练、未来预约、实际历史、累计节数 / 小时、课程细分，以及支持“月 / 年 / 全部”和“节数 / 小时”切换的单轴折线图；前端只读本地缓存，缺数、Session 失效、缓存过期和同步错误必须分别展示。
- 阶段 1B｜Home 摘要：数据准确且连续同步稳定后，增加 `home-lane-rail` / `widget-wide` 芭蕾入口，位于 Today Todo 后、豆奶前，只显示下一节课、本周进度和同步状态；失败时继续显示最后成功内容并标旧。
- 阶段 2｜学习闭环：从 personal-wiki 单向同步阶段目标、课堂重点、老师纠正、动作标签和练习计划；MaxNow 不提供编辑后端，也不把可选身体状态放进 Home。
- 阶段 3｜约课 dry-run：配置课程名、老师、日期 / 时段、优先级、每周上限、冲突规则和“满员后是否候补”，连续覆盖至少两次周日 14:20 发布并只输出预计动作。
- 阶段 4｜受控自动约课：只有 Session 生命周期结论明确、dry-run 与人工意图一致且 Owner 单独批准后才启用；14:20 前只读预检，提交时有限重试并使用幂等账本，结果区分正式成功、候补成功、已预约、规则拒绝、登录失效和未知错误。
- 当前 Session 实验仍只回答“持续活动时最多能维持多久”，不证明静默闲置寿命；具体 unit、日志和停止方式以 `SERVER_RUNBOOK.md` 为唯一运维真相。
- Home 不展示完整课表、历史或运维日志；Cloud 仅展示采集和自动化健康。
- `PHPSESSID` 视为约课网站密码：只允许进入服务器隔离凭据，不得进入 Git、前端、环境变量、命令参数、日志、备份或聊天；Cookie、OAuth code、微信标识、手机号、会员卡号和原始响应正文同样禁止外露。
- 默认不自动取消、转课、买卡、支付或报名付费活动；验证码、微信重新授权、页面结构变化或无法判断的响应一律 fail closed。
- 开始实现前还需 Owner 确认当前级别、每周目标、主要课程 / 老师 / 时段偏好、候补和冲突规则、学习笔记格式及通知渠道；导航位置、粉白视觉、Home 摘要边界和统计口径已确认，不再作为阻塞问题。
- 只读验收：同一批源数据重跑后无重复；补录 / 改课可在滚动窗口内正确更新；累计节数与分钟等于明细汇总；月 / 年空档补零；失败不会清空上次成功数据；`720px` / `768px` 高度和 `860px` / `390px` 宽度下侧栏与图表无整页溢出。
- 总体验收顺序：先做到预约与上课记录准确、来源和更新时间可信；再让 dry-run 连续两周与人工选择一致；最后才讨论真实提交。自动预约成功不等于学习模块完成。
