...
- 芭蕾“课程计划”宽桌面左侧预约占三分之一并贯穿模块，右侧顶部助手 / 上次结果等分、下方周安排占满；下方完整本周课程表继续保留。课程预约按完整自然日展示最近三个有课日期，精确取消截止退出快照后显示可信的过期 / 已结束状态。周安排按日期、时间与标准课型语义合并预约、课表和 Fast Path 结果，真实课程复用完整课表样式，支持上周、本周、下周三个周一至周日周期；下周执行前显示带小字号真实优先级的待抢目标，执行后由同周真实结果替换。上次结果只保留成功预约、候补中、未抢到总体数量。
