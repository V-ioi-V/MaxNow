...
- 2026-07-05 只读检查线上 `dash/data/dounai_checkin.json`、root 豆奶签到脚本和账号快照生成脚本。
- 当前线上数据能确认：签到奖励记录有 61 天；账号余量快照 `account_history` 只有 2026-06-21 到 2026-07-05 共 15 天，因此还不能形成完整 30 天真实消耗序列。
- 现有脚本只抓取 `https://dounai.pro/user/panel` 的剩余流量、账号有效期、VIP 有效期，并维护每日账号余量快照；未发现已经接入账单、明细、历史使用量或每日消耗量接口。
- 当前登录态实时打开用户面板会跳到 `https://dounai.pro/chaoshi.html`，但当天 09:00 cron 仍已成功写入账号快照；后续若要继续查页面菜单或接口，需要先确认 / 刷新登录态。
- 用相邻账号余量快照只能做估算：`估算消耗 = 前一天剩余 + 当天签到获得 - 当天剩余`。该口径会受剩余流量显示精度、签到奖励、充值 / 赠送、套餐调整、有效期变化和快照时间影响。
- 2026-06-21 到 2026-07-05 的可用快照显示净减少约 30.0GB，同期签到奖励约 6.7GB；按差分估算，窗口内消耗约 37.4GB，但这是 15 天估算，不是 30 天真实账单。
- v1 暂不新增前端写入、账号操作或手动录入口；如果要展示“近 30 天用量”，建议等 `account_history` 累积满 30 天，或先找到豆奶站点可读的真实用量 / 明细接口。
