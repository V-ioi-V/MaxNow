...
- 阶段 0｜数据契约：整理脱敏样本，确定课程 / 场次 / 预约 / 上课记录稳定键、北京时间口径、状态枚举、去重规则和新鲜度状态；确认 personal-wiki 中目标与笔记的唯一来源格式。
- 阶段 1｜只读 MVP：实现候选独立页面“芭蕾 / 学习与约课”，展示下一节课、本周训练、未来预约、上课历史和学习统计；前端只读本地缓存，缺数、Session 失效和同步错误必须显式展示。
...
- Home 在只读数据稳定后最多增加“下一节课 + 本周进度 + 数据状态”紧凑入口，不展示完整课表、历史或运维日志；Cloud 仅展示采集和自动化健康。
...
- 开始实现前还需 Owner 确认当前级别、每周目标、主要课程 / 老师 / 时段偏好、候补和冲突规则、学习笔记格式、Home 摘要口径及通知渠道。
- 验收顺序：先做到预约与上课记录准确、来源和更新时间可信；再让 dry-run 连续两周与人工选择一致；最后才讨论真实提交。自动预约成功不等于学习模块完成。
