...
- 阶段 0A-1B｜只读数据与页面：已部署到 MaxNow；每日 00:17 rolling 与每月 1 日 00:47 full timer 已在 2026-07-27 经 Owner 批准并启用，刷新预约、候补、上课记录和课程卡。登录失效后停止请求并保留旧数据。
...
- 当前 Session 实验仍只回答“持续活动时最多能维持多久”，不证明静默闲置寿命；生产 00:17 与月度 full timer 已启用，具体 unit、日志和停止方式以 `SERVER_RUNBOOK.md` 为唯一运维真相。
...
- 独立芭蕾页已覆盖下一节与所有预约、候补序号、真实取消截止、实际上课历史、整体训练统计、课程卡与本周训练摘要；课程卡按近 28 天实际上课频率判断到期前能否用完，并显示建议周课次与需要增加的频率。
- 闻道只读采集器仅允许已确认的 GET 页面，预约、候补、上课记录与课程卡分别进入脱敏快照和 canonical ledger；前端不包含 PHPSESSID、Cookie、会员卡号、会员 ID、源记录 ID 或原始响应。
- 服务器使用 host-bound systemd 加密凭据、enable gate、单实例锁和 fail-closed 身份阻断。每日 00:17 rolling 与每月 1 日 00:47 full timer 已启用；身份失效立即停止后续请求，保留最后成功数据，直到安全凭据版本变化。
