...
- 阶段 0A-1B｜只读数据与页面：已部署到 MaxNow；每天 09:00 / 12:00 / 15:00 / 18:00 / 22:00 rolling、周日额外 14:30 课表刷新与每月 1 日 00:47 full timer 已启用，刷新预约、候补、上课记录、课程卡和周课表；每周期最后一节结束后 10 分钟另做收尾刷新，20 分钟时生成周简报。登录失效后停止请求并保留旧数据。
...
- 阶段 2.6｜芭蕾数据型分享图：每周小红书 `week N` 封面与训练周简报已经完成，见 Done。后续可把 Owner 选定的下一节课、本月 / 年度节数与时长、主要课型和近期训练记录排成更多可选模板；所有分享图均只读取脱敏 read model，不上传服务器，也不包含 PHPSESSID、会员 / 卡片标识、源记录 ID、自动化内部状态或未经选择的私人信息。
- 阶段 3-4｜无人值守自动抢课：Owner 已明确批准长期周规则与周日 14:20 执行；周一至周五只处理 18:40（含）后开始的标准芭蕾 L1 与精确“软开 / 软开课”，周六全天，不再依赖固定课表。fast path、私有幂等账本、逐课安全结果和 MaxNow 状态面板已完成，见 Done。以后扩展课程类型或日期仍需明确配置、测试和 Owner 授权。
- 当前 Session 实验仍只回答“持续活动时最多能维持多久”，不证明静默闲置寿命；生产每天 09:00 / 12:00 / 15:00 / 18:00 / 22:00、周日额外 14:30、每周期动态收尾检查与月度 full timer 已启用，具体 unit、日志和停止方式以 `SERVER_RUNBOOK.md` 为唯一运维真相。
...
- Owner 确认实际上课历史中未填写老师的记录默认按“李俊”处理；2026-08-07 19:45–21:15 芭蕾 L1 明确校正为“张瀚泽”，闻道明确姓名和其他 Owner 手工补录姓名保持不变。
- Dashboard 同步、老师聚合和实时上课查询共用同一归一化规则；未来课表、当前预约和候补不套用默认老师。
- 2026-08-14 22:57 实时只读复核闻道全部上课记录：9 条闻道记录完整可见；叠加 2 条 Owner 已确认的李俊手工补录，并应用 8 月 7 日张瀚泽校正后，MaxNow 共 11 节。

### 已完成芭蕾训练周简报

- `week N` 弹窗改为双页图片浏览器：可用顶部切换按钮或左右滑动，在原周封面与训练周简报之间切换；复制图片和下载 PNG 始终作用于当前页。
- 周简报保留固定底图中的标题、“第 / 周”、六项指标名和装饰，浏览器只填周数、日期、本周训练次数 / 时长 / 最爱课程、总次数 / 时长 / 最爱课程；六项数据直接复用训练概览统计函数，只纳入截止时间前已经上完的课。
- 周简报以每个周一至周日周期最后一节已预约或实际完成课程的结束时间为统计截止点；结束后 10 分钟刷新全部芭蕾数据，再过 10 分钟由浏览器自动生成。页面未打开时下次进入立即生成，不保存周简报成品，也不进入 14:20 自动抢课关键路径。
- 首次线上空白已修复：运行时两张底图改为轻量 WebP，动态字改为同款 WOFF2 网页子集，并且只生成当前页；完整 PNG / TTF 继续作为源文件保留。
- 动态周数、日期、数字和课程名统一使用项目内置 `Ma Shan Zheng` 字体；字体文件与 OFL 许可随项目保存，避免服务器或浏览器字体差异。
- 已验证 `1280×900` 与 `390×844`：双页画布等宽等高、弹窗和页面无横向溢出，按钮切换、左右滑动、复制和下载可用。
...
- 芭蕾页按学习决策顺序收敛为“顶部训练 / 课程卡 / 等级概览 → 课程计划 → 训练记录”；课程计划用预约、三周周安排、抢课助手和上次结果四区集中当前课程、待抢目标与执行结果。
...
- 北京时间每周日 14:19:35 启动服务器本地服务并预热当前闻道 Session，14:20:00 准点执行长期周规则；关键路径不经过 Codex、Skill 或 SSH。每次执行结束后，无论成功或失败，都立即触发一次独立 GET-only rolling 同步刷新 MaxNow 芭蕾面板，刷新结果不覆盖抢课结果。
- 当前目标为周一至周五 18:40（含）后的全部标准芭蕾 L1、标准芭蕾 L1.5 与精确“软开 / 软开课”，以及周六全天同类课程；周日、软开专项和软开-胯排除。课程层顺序为 L1 → L1.5 → 软开，每类先周六、再周一至周五；老师不限，教室大教室优先、小教室兜底。六天课表第一轮最多 3 路并发读取，放课后第 2 / 6 / 10 秒再以最多 2 路 GET 后台刷新完整日期集合；已发现 L1 与刷新流水线重叠执行，稳定快照补入后发布课程后再处理 L1.5 / 软开。课程卡与规则最多 2 路并发预检且 15 秒过期，HTTPS 使用最多 3 条 keep-alive 连接；真实预约 / 候补 mutation 仍严格串行。单节失败或 mutation 结果未知仍继续后续课程，未知 mutation 本身不重复提交，统一核验未命中后仅在第 0.8 / 2 / 4 秒追加只读复核。身份失效或页面协议变化仍全局停止。
- 2026-08-23 已完成分批放课修复与整体流水线优化：fixture 首次只出现 L1 / 软开、后续出现周四和周六 L1.5 时，18 节目标完整发现且全部 L1.5 先于软开提交；另一 fixture 证明首个 L1 mutation 在后台发现窗口结束前开始。未知候补响应延迟到第三次核验才可见时，仍保持单课一次 mutation 并正确回填候补。
...
- 芭蕾“课程计划”宽桌面左侧预约占三分之一，右侧独立按自然高度排列顶部助手 / 上次结果和下方周安排；下方完整本周课程表继续保留。课程预约按完整自然日展示最近三个有课日期，精确取消截止退出快照后显示可信的过期 / 已结束状态。周安排按日期、时间与标准课型语义合并预约、课表和 Fast Path 结果，真实课程复用完整课表的左侧时间轴、空时段压缩、分钟定位、课程元素和状态样式，支持上周、本周、下周三个周一至周日周期；下周执行前显示带小字号真实优先级的待抢目标，执行后由同周真实结果替换。上次结果只保留成功预约、候补中、未抢到总体数量。
...
- 服务器使用 host-bound systemd 加密凭据、enable gate、单实例锁和 fail-closed 身份阻断。每天 09:00 / 12:00 / 15:00 / 18:00 / 22:00 rolling、周日 14:30 额外刷新、自动抢课执行后即时刷新与每月 1 日 00:47 full timer 已启用。身份失效立即停止后续请求，保留最后成功数据，直到安全凭据版本变化。
...
- 线上 `/var/www/maxnow-dashboard/dash/data/dounai_checkin.json` 已新增 `traffic_usage` 和 `traffic_usage_history`；签到改由 Owner 手动完成，服务器 00:05 日结以 GET-only 方式回读手动签到变更、账号余量和真实使用量。
