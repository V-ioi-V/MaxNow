...
- 来源：2026-08-23 Fast Path 在 14:20 执行时漏掉周四晚与周六下午的 L1.5；当天只要先出现任意 L1 / 软开，日期级预取就提前收口，没有等待同日课程发布稳定。
- 建议分支：`bugfix/ballet-fast-progressive-release`
- 日期页不能以“出现任意一个匹配目标”作为发布完成条件；在有变化时继续短窗口轮询，直到匹配集合连续稳定或达到明确截止，同时仍保持最多 3 路课表 GET、真实 mutation 严格串行。
- 增加分批 fixture：第一次只出现 L1 / 软开，后续才出现 L1.5；最终必须把完整集合按 `L1 → L1.5 → 软开`、每层周六优先再周一至周五提交。
- 验收必须覆盖 `mutationAttempts=0` 的生产 dry-run、发布集合稳定证据和旧规则不回归；不得用本周手工补抢结果掩盖发现阶段漏课。
...
- 阶段 0A-1B｜只读数据与页面：已部署到 MaxNow；每天 09:00 / 12:00 / 15:00 / 18:00 / 22:00 rolling、周日额外 14:30 课表刷新与每月 1 日 00:47 full timer 已启用，刷新预约、候补、上课记录、课程卡和周课表；周简报复用周日 18:00 的 rolling 数据。登录失效后停止请求并保留旧数据。
...
- 当前 Session 实验仍只回答“持续活动时最多能维持多久”，不证明静默闲置寿命；生产每天 09:00 / 12:00 / 15:00 / 18:00 / 22:00、周日额外 14:30 / 20:00 与月度 full timer 已启用，具体 unit、日志和停止方式以 `SERVER_RUNBOOK.md` 为唯一运维真相。
...
- 周简报使用最近一个已经到达的北京时间周日 18:00 作为统计截止点；直接复用每天 18:00 的既有只读 rolling 同步，不保存周简报成品，也不进入 14:20 自动抢课关键路径。
...
- 当前目标为周一至周五 18:40（含）后的全部标准芭蕾 L1、标准芭蕾 L1.5 与精确“软开 / 软开课”，以及周六全天同类课程；周日、软开专项和软开-胯排除。课程层顺序为 L1 → L1.5 → 软开，每类先周六、再周一至周五；老师不限，教室大教室优先、小教室兜底。六天课表最多 3 路并发读取，课程卡与规则最多 2 路并发预检且 8 秒过期，HTTPS 使用最多 3 条 keep-alive 连接；真实预约 / 候补 mutation 仍严格串行，最终预约详情并发只读核验。发布延迟、临时网络失败和明确未开放可安全重试 3 次，单节失败或 mutation 结果未知仍继续后续课程，未知 mutation 本身不重复提交。身份失效或页面协议变化仍全局停止。
