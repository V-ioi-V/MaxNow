...
- 固定 25 分钟阶段已在 2026-07-26 23:28 的首个标准检查返回登录失效并停止：最后认证样本为 23:03，已确认有效时长冻结为 3 小时 56 分 06 秒。生产每日 00:17 抓取仍保持“待启用”，下一步是 Owner 在电脑微信重新登录、安全替换加密凭据；新会话探针按 Owner 最新要求改回每 20 分钟，再单独批准首次只读生产验证和 timer 启用。
- 服务器已在内部密封 host-bound systemd 加密凭据，并安装每日 00:17 rolling / 每月 1 日 00:47 full 的 unit；`LoadCredentialEncrypted`、非敏感凭据版本和 enable gate 共同控制运行。当前 timer / gate 仍关闭；`PHPSESSID`、Cookie 和原始响应不得进入 Git、日志、前端、聊天、环境变量或命令参数。
- 身份失效时立即停止网络重试、保留旧缓存，并展示安全错误与“请在电脑微信重新登录并刷新凭据”；只有凭据版本变化后才允许再次尝试。
- 三阶段实验只证明该 PHPSESSID 在 23:03 仍有效、23:28 已失效；10 / 20 分钟活动频率没有把它长期维持，也不能据此推断精确空闲寿命、滑动过期或长期自动续期。
