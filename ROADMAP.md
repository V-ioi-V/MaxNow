...
- 阶段 3｜约课 dry-run：配置单节或多节课程清单、老师、日期 / 时段、优先级、每周上限、冲突规则和“满员后是否候补”，连续覆盖至少两次周日 14:20 发布并只输出预计动作。
- 阶段 4｜受控自动约课：只有 Session 生命周期结论明确、dry-run 与人工意图一致且 Owner 单独批准后才启用；14:20 前只读预检，提交时有限重试并使用幂等账本，结果区分正式成功、候补成功、已预约、规则拒绝、登录失效和未知错误。多课请求先统一预检，再逐节顺序提交并返回每节独立结果；不把多节课伪装成原子事务，身份失效或结果不明确时停止剩余提交。
