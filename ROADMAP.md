...
- 服务器已用 host-bound systemd 加密凭据保存 v6 探针和生产同步所需的最小凭据，并安装每日 00:17 rolling / 每月 1 日 00:47 full 的 unit；`LoadCredentialEncrypted`、非敏感凭据版本和 enable gate 共同控制运行。Owner 已批准并完成一次受控 rolling 只读同步，真实缓存现有 1 条上课记录和 3 条未来预约；生产 timer / gate 仍关闭。`PHPSESSID`、Cookie 和原始响应不得进入 Git、日志、前端、聊天、环境变量或命令参数。
