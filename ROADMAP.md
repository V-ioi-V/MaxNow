...
- 阶段 0A-1B｜只读数据与页面：已部署到 MaxNow；每日 00:00 rolling、周日 14:30 课表刷新与每月 1 日 00:47 full timer 已启用，刷新预约、候补、上课记录、课程卡和周课表。登录失效后停止请求并保留旧数据。
...
- 阶段 2.6｜芭蕾分享图：在芭蕾页把 Owner 选定的关键数据排成适合保存 / 分享的图片；候选内容包括下一节课、本周训练、本月 / 年度节数与时长、主要课型和近期训练记录。第一版优先做浏览器本地生成与下载，提供生成前预览和信息开关，不上传服务器；只读取现有脱敏 read model，不包含 PHPSESSID、会员 / 卡片标识、源记录 ID、自动化内部状态或未经选择的私人信息。具体图片比例、视觉模板和默认字段在实现前再确认。
...
- 当前 Session 实验仍只回答“持续活动时最多能维持多久”，不证明静默闲置寿命；生产每日 00:00、周日 14:30 与月度 full timer 已启用，具体 unit、日志和停止方式以 `SERVER_RUNBOOK.md` 为唯一运维真相。
...
- 芭蕾顶部新增“成长进度”卡：宽屏与本周训练、课程卡组成三等分概览，中等宽度独占下一行，窄屏单列。
- 升班路径与阶段效果以 private personal-wiki 的李俊芭蕾课程指南为语义来源，按实际上课记录计算当前级别课次、基础月份与 100 分升班参考；达到参考只提示咨询老师测评。
- 全部已完成训练按课程定位累计 XP，并映射为发力启蒙、单一元素、枢纽建立、协调开范、组合流畅、认知串联六级成长效果；L5 已纳入课程分类器。

### 已完成芭蕾模块信息架构收敛

- 芭蕾页按学习决策顺序重排为“下一节 + 本周训练 → 课程计划 → 本周课程表 → 训练记录 → 课程卡”，课程目标、预约 / 候补和逐课结果集中在课程计划中。
...
- 北京时间每周日 14:19:35 启动服务器本地服务并预热当前闻道 Session，14:20:00 准点按固定优先级 `周六 > 周日 > 周五 > 其他日期` 顺序预约；关键路径不经过 Codex、Skill 或 SSH。
- 当前固定目标为周六 11:30–12:30 李俊软开大教室、周五 19:45–21:15 李俊芭蕾 L1 大教室、周二 19:45–21:15 李俊芭蕾 L1 大教室。每节独立即时校验；发布延迟、临时网络失败和明确未开放可安全重试 3 次，单节失败或 mutation 结果未知仍继续后续课程，未知 mutation 本身不重复提交，全部结束后统一实时核验。身份失效或页面协议变化仍全局停止。
- 私有状态维护幂等 occurrence 与累计成功数；公开状态只展示启用状态、计划、上次 / 下次执行、目标、逐课结果和累计数，经登录保护的 `no-store` alias 提供给芭蕾课程计划与 Cloud 运维卡。自动候补、取消、转课、支付和登录继续禁止。
...
- 服务器使用 host-bound systemd 加密凭据、enable gate、单实例锁和 fail-closed 身份阻断。每日 00:00 rolling、周日 14:30 额外刷新与每月 1 日 00:47 full timer 已启用；周日只读刷新与 14:20 自动抢课错开。身份失效立即停止后续请求，保留最后成功数据，直到安全凭据版本变化。
