...
- 只读模块已落地；导航顺序固定为首页 → 豆奶 → Token → 芭蕾 → 云服务 → 生活 → 同行记，芭蕾页使用粉玫瑰 + 白卡语义。
- 已落地范围：脱敏 `ballet.*` read model、只读页面与 Home 摘要、下一节预约与完整“所有预约”列表、统一“本月 / 今年 / 全部 × 节数 / 时间”统计、同步切换的课程类型 / 级别细分与趋势、预约与实际上课记录分离、Owner 手工记录、增量账本 / 去重 / 原子缓存设计，以及最近成功和安全失败状态。
- 芭蕾页新增独立的 PHPSESSID 活跃实验状态卡：显示截至最后认证样本的已确认有效时长、原始起始时间、最近 / 下次自动检查和当前间隔；状态由服务器本地日志脱敏发布，不会刷新课程数据时间，也不把 Session、指纹或内部路径送进前端。
- 同步器只允许已确认的闻道 GET 页面，不包含预约、取消、候补、转课或其他写操作；相同数据重跑必须幂等，失败保留最后成功缓存。
- 固定 25 分钟的旧会话阶段已在 2026-07-26 23:28 确认登录失效并停止。Owner 于 2026-07-27 重新打开闻道页面并退出微信后，本机安全提取出新一代会话；服务器 v6 只读探针自 00:26:55 起按每 20 分钟运行，首条样本为 HTTP 200 / authenticated。新旧会话实验必须分别计时，不能把旧会话的 3 小时 56 分 06 秒并入 v6。
- 服务器已用 host-bound systemd 加密凭据保存 v6 探针和生产同步所需的最小凭据，并安装每日 00:17 rolling / 每月 1 日 00:47 full 的 unit；`LoadCredentialEncrypted`、非敏感凭据版本和 enable gate 共同控制运行。Owner 已批准并完成一次受控 rolling 只读同步，真实缓存现有 3 条未来预约；另按 Owner 明确事实补入 1 条手工软开课，上课历史合计 2 节 / 2.5 小时。生产 timer / gate 仍关闭。`PHPSESSID`、Cookie 和原始响应不得进入 Git、日志、前端、聊天、环境变量或命令参数。
- 身份失效时立即停止网络重试、保留旧缓存，并展示安全错误与“请在电脑微信重新登录并刷新凭据”；只有凭据版本变化后才允许再次尝试。
- v3-v5 三阶段只证明旧 PHPSESSID 在 23:03 仍有效、23:28 已失效；v6 是独立的新凭据实验。12:01 前由固定 20 分钟课程列表探针构成，12:01 起包含一次额外预约 / 上课记录只读同步；任何正常样本都只证明该次检查时有效，不能单独推断精确空闲寿命、滑动过期或长期自动续期。
...
- 阶段 0A-1B｜只读数据与页面：已部署到 MaxNow；2026-07-27 首次受控真实同步已成功，当前缓存包含实际上课记录和未来预约。每日 / 每月 timer 仍待 Owner 单独批准，不能把一次手动同步描述成定时抓取已经运行。
...
- 当前 Session 实验仍只回答“持续活动时最多能维持多久”，不证明静默闲置寿命；生产 00:17 timer 保持未启用，具体 unit、日志和停止方式以 `SERVER_RUNBOOK.md` 为唯一运维真相。
