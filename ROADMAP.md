...
- 当前目标为周一至周五 18:40（含）后的全部标准芭蕾 L1 与精确“软开 / 软开课”，以及周六全天同类课程；周日、软开专项和软开-胯排除。顺序为全部 L1 后全部软开，每类先周六、再周一至周五；老师不限，教室大教室优先、小教室兜底。六天课表最多 3 路并发读取，课程卡与规则最多 2 路并发预检且 8 秒过期，HTTPS 使用最多 3 条 keep-alive 连接；真实预约 / 候补 mutation 仍严格串行，最终预约详情并发只读核验。发布延迟、临时网络失败和明确未开放可安全重试 3 次，单节失败或 mutation 结果未知仍继续后续课程，未知 mutation 本身不重复提交。身份失效或页面协议变化仍全局停止。
