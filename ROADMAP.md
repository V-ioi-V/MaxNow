...
- 服务器已用 host-bound systemd 加密凭据保存 v6 探针和生产同步所需的最小凭据，并安装每日 00:17 rolling / 每月 1 日 00:47 full 的 unit；`LoadCredentialEncrypted`、非敏感凭据版本和 enable gate 共同控制运行。生产 timer / gate 仍关闭，v6 只验证固定课程列表 GET；`PHPSESSID`、Cookie 和原始响应不得进入 Git、日志、前端、聊天、环境变量或命令参数。
- 身份失效时立即停止网络重试、保留旧缓存，并展示安全错误与“请在电脑微信重新登录并刷新凭据”；只有凭据版本变化后才允许再次尝试。
- v3-v5 三阶段只证明旧 PHPSESSID 在 23:03 仍有效、23:28 已失效；v6 是独立的新凭据实验。任何正常样本都只证明该次检查时有效，不能单独推断精确空闲寿命、滑动过期或长期自动续期。
...
- 阶段 0A-1B｜只读数据与页面：已部署到 MaxNow；生产抓取仍待新凭据、首次受控验证和 Owner 单独批准后启用，不能把样例缓存当作每日同步已运行。
