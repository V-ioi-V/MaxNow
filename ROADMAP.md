...
- 阶段 2.6｜芭蕾分享图：第一类已确认是每周小红书“芭蕾课周记录”封面，固定 `1280×1710` 粉色手作模板，2026-08-02 所在周为 `week 2`，按北京时间每逢周日加一；使用已完成的透明手绘 `0–9` 数字素材在浏览器本地拼图，不每周重新调用 AI，也不依赖系统字体。下一步在芭蕾页顶部加入紧凑 Week 入口、封面预览、下载和剪贴板复制，并完成桌面 / 手机验证。后续数据型分享图仍可把 Owner 选定的下一节课、本周训练、本月 / 年度节数与时长、主要课型和近期训练记录排成可选模板；所有分享图均只读取脱敏 read model，不上传服务器，也不包含 PHPSESSID、会员 / 卡片标识、源记录 ID、自动化内部状态或未经选择的私人信息。
...
- 本项只完成长期可复用素材；周日自动加一、封面底图、芭蕾页 Week 入口、下载和剪贴板复制仍按 Later 阶段 2.6 推进。
