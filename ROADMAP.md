...
- 当前固定目标为周五软开、周五芭蕾 L1、周二软开、周二芭蕾 L1、周四软开，均保持原晚间时段和大教室但不限老师。每节按日期、课型 / 等级、起止时间和教室独立即时唯一匹配；老师不进入匹配或 occurrence 幂等键。发布延迟、临时网络失败和明确未开放可安全重试 3 次，单节失败或 mutation 结果未知仍继续后续课程，未知 mutation 本身不重复提交，全部结束后统一实时核验。身份失效或页面协议变化仍全局停止。
