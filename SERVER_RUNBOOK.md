...
芭蕾页面与脱敏缓存已经部署。生产 rolling timer 每天 09:00、12:00、15:00、18:00、22:00 刷新全部数据，并在周日 14:30 额外读取抢课后发布的下周课表。独立 `maxnow-ballet-week-closeout.timer` 每 5 分钟只检查公开脱敏 read model；到本周期最后一节结束后 10 分钟且数据尚未刷新时，才启动一次既有 rolling 完整只读同步，浏览器于结束后 20 分钟生成周简报。检查器不读取凭据、不访问网络，每周期最多尝试 3 次；登录失效时失败关闭。周日 14:30 触发与 14:20 自动抢课关键窗口错开，原每日 00:00 和固定周简报触发均已移除。每月 1 日 00:47 继续执行 full 只读同步。长期 enable gate 为 root `0600`，三个只读相关 timer 均应保持 `enabled / active / waiting`。
...
closeout service -> maxnow-ballet-week-closeout.service（只读公开模型，无网络 / 无凭据）
closeout timer -> maxnow-ballet-week-closeout.timer（每 5 分钟检查；最后一节 +10 分钟启动 rolling）
closeout state -> /var/lib/maxnow-ballet-week-closeout/state.json
...
4. 创建 root 管理的 `/etc/maxnow-ballet/enable-sync` 后，启用并启动 `maxnow-ballet-sync.timer`、`maxnow-ballet-week-closeout.timer` 与 `maxnow-ballet-full-sync.timer`。确认 rolling timer 包含每天 09:00、12:00、15:00、18:00、22:00 和周日额外 14:30，不再包含每日 00:00 或固定周简报触发；closeout 每 5 分钟检查脱敏 read model，只在最后一节结束后 10 分钟启动 rolling，full timer 为每月 1 日 00:47。不要把测试性即时请求算作定时任务证据。
...
Owner 已单独批准长期周规则在北京时间每周日 14:20 无人值守预约。关键路径不经过 Codex、Skill、OpenClaw 或 SSH：
...
长期规则为：放课后扫描周一至周六课表；周一至周五只处理 18:40（含）后开始的课；周六按结束时间判断，只处理结束时间严格早于 18:00 的课，18:00 整结束也排除。课程范围为所有标准芭蕾 L1、标准芭蕾 L1.5，以及课程名归一化后严格等于“软开”或“软开课”的课程；周日、软开专项、软开-胯和其他近似名称均排除。先按日期优先级周六 → 周二 → 周五 → 周一 → 周三 → 周四，再在每天内部按 L1 → L1.5 → 软开 / 软开课排序；工作日同一天同课型先李俊（老师空白按李俊）、再其他老师，各组内按开始时间排序。周六不按老师分层；工作日同名同时段先选李俊老师层，未命中才选其他老师层，每层内仍先在大教室中唯一匹配，大教室为 0 节时再在小教室中唯一匹配。

服务在 14:19:35 先用当前日期课表做一次只读会话预热；14:20 后最多 3 路并发读取周一至周六 6 个日期页。第一轮只让已发现的周六 L1 提前进入串行处理，同时后台在放课后第 2 / 6 / 10 秒以最多 2 路 GET 刷新全部日期；每个日期保留目标数不减少的最新页面，进入周六 L1.5 / 软开或任何后续日期前必须等待发现窗口结束，再按稳定快照补入周六剩余课程并按日期优先级处理后续各天。课程卡资格和闻道规则由最多 2 个 worker 提前预检，完成超过 15 秒则在提交前重新预检。HTTPS 连接池最多保留 3 条 keep-alive 连接，整次服务上限 600 秒，以容纳六天动态规则可能发现的多节课程。真实 `do_addbook` 始终严格串行，任意时刻最多一个预约 / 候补 mutation；读取、发现和预检只在安全的只读路径并行。匹配字段为日期、实际课程名、课型 / 等级和起止时间；老师不参与 occurrence 幂等键，但工作日候选选择与提交排序使用老师优先层。工作日同名同时段先限定李俊（含空老师），未命中才选择其他老师；每个老师层内先限定大教室并要求恰好一节，大教室为 0 节时再限定小教室并要求恰好一节；周六保持不分老师、按教室优先。优先层出现多节时仍失败关闭，不猜测提交。`available` 执行预约；`queue_available` 只有在仓库配置显式设置 `allowWaitlist=true` 时执行候补；已预约或已排队不重复提交。网络波动、课程卡尚未开放、规则短暂未就绪或 mutation 明确返回 `NOTOPEN` 时，首次失败后最多重试 3 次，间隔为 80 / 160 / 320ms；明确已满、已停止、无卡或不可约时直接进入下一节。

每节是独立失败域：一节失败或 mutation 结果未知都继续后续目标。未知 mutation 可能已经被闻道接受，因此本节绝不重复 POST；全部课程提交后先统一查询实时预约记录，仍未命中的未知目标再按提交后第 0.8 / 2 / 4 秒进行限定目标日期的只读复核。若查到则按 `bookingStatus=booked` 或 `waitlist` 回填，并发布正整数候补位次与 `verificationAttempts`；始终未查到才保留“结果待确认”。只有 `auth_required`、配置错误或页面结构变化等全局问题才停止后续课程。fast path 单请求超时为 5 秒，避免一次网络故障长期占住后续高优先级目标。

Fast Path 单元进入成功或失败终态后，systemd 分别通过 `OnSuccess=maxnow-ballet-sync.service` / `OnFailure=maxnow-ballet-sync.service` 立即排队一次既有 rolling 同步。该同步保持业务只读：除既有 GET allowlist 外，只允许闻道上课记录页面自身声明的固定 `newcheckrecord/{storeId}/{offset}` 与预约记录页面声明的固定 `newbookrecord/{storeId}/{offset}` 分页 POST；两个 POST 都只加载列表分页，不执行预约、候补、取消或转课。同步、实时查询和 Fast Path 复核共用活动预约分页读取器，校验总数、`customerid` 与路径 contract，按已加载条数翻页、跨页合并去重后才读取活动详情。同步负责刷新 `dash/data/ballet.*` 中的预约、候补、上课记录、课程卡与课表；它在真实 mutation 与统一核验之后运行，不占用 14:20 关键路径，也不改变 Fast Path 单元自身的 Result。若 rolling 同步已经运行，systemd 合并 start job，不并发启动第二个实例。部署验收不得手动启动 Fast Path；可用 `systemd-analyze verify` 和 unit 属性检查完成钩子，只有实际定时执行后才验收自动联动。
...
2026-08-03 已部署芭蕾周简报龙藏体：

```text
deployed source commit: 04c7582 Use Long Cang for ballet weekly brief
version: 1.0.9.04
changes: 周简报动态周数、纯数字日期、次数、时长和课程名改用 Long Cang（龙藏体），并增加同色轻量圆润描边；底图固定标题与指标名继续保留，不重绘
asset cache: styles.css?v=239；app.js?v=197；LongCang-Weekly.woff2 为 53112 bytes，完整 TTF、子集字符清单和 OFL 许可保留
runtime data backup: /home/ubuntu/maxnow-deploy-backups/20260803-233322-ballet-week-long-cang/dash-data.tgz
runtime data stash: 56ddaeaddd4fa3f0948d7e55d81c48e2f4ed935b（部署后保留；服务器权威 dash/data 已恢复，project-meta.* 按新版本重新生成）
deployment path: origin/main 快进到 04c7582，再通过校验过的 Git bundle 将服务器 main 从 c99ec5b 快进；未覆盖或刷新芭蕾课程数据
local visual verification: 1280×720 与 390×844 真实弹窗均生成成功，龙藏体与轻描边已应用；画布保持 1280×1710，纯数字日期与周数基线未变，375px 可用宽度下无横向溢出
...
2026-08-16 已部署周安排时间轴收口修复：

```text
deployed source commits: ac23183 fix: trim ballet weekly timeline + 905a974 chore: refresh project metadata
version: 1.0.10.13
changes: 压缩空档只显示 13:00 起始刻度，不再在下沿显示 18:00；时间轴最后一小时按全周最晚课程的准确结束分钟裁短，最后课程 21:15 结束时课表同步收在 21:15
runtime backup: /home/ubuntu/maxnow-deploy-backups/20260816-ballet-timeline-trim/dash-data.tgz
...
```text
deployed source commit: 91b30d6 fix(ballet): separate booked compact status
version: 1.0.10.31
changes: 保持完整课表与一小时卡紧凑高度不变；60 分钟及以下课程改为 9px 课程名、7px 老师 / 人数 / 时间，正式预约状态从时间右侧移到卡片最底部左对齐独占一行
runtime backup: /home/ubuntu/maxnow-deploy-backups/20260823-164426-ballet-compact-booked-row；部署 stash predeploy-ballet-compact-booked-row-20260823-164426 保留
browser verification: 2048 / 1920 / 1800 / 1501 / 1500px 下一小时正式预约卡均为 column 底栏，时间与状态不在同一行，状态距卡片底边约 3.7px；23 张一小时卡统一为 9px / 7px，关键内容无裁切，整页横向溢出为 0
server verification: scripts/check.py 与 nginx -t 全部通过；styles.css?v=257 生效；服务器 HEAD 与 origin/main 一致；匿名首页 / 登录页 / 芭蕾数据 / Blog 为 302 / 200 / 401 / 200
automation state: nginx、maxnow-auth、rolling / full / Fast Path timers 均 active；rolling / full / Fast Path service 均 inactive
safety: 静态 UI 部署和验收未访问闻道、未启动芭蕾同步或 Fast Path，未执行预约、候补、取消或转课
```
