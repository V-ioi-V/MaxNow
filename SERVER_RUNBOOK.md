...
长期规则为：放课后扫描周一至周六课表；周一至周五只处理 18:40（含）后开始的课；周六按结束时间判断，只处理结束时间严格早于 18:00 的课，18:00 整结束也排除。课程范围为所有标准芭蕾 L1、标准芭蕾 L1.5，以及课程名归一化后严格等于“软开”或“软开课”的课程；周日、软开专项、软开-胯和其他近似名称均排除。课程层固定为全部芭蕾 L1、全部芭蕾 L1.5、全部软开 / 软开课；每层依次为周六、周一至周五李俊（老师字段为空按李俊）、周一至周五其他老师，各组内按日期和开始时间排序。周六不按老师分层；工作日同名同时段先选李俊老师层，未命中才选其他老师层，每层内仍先在大教室中唯一匹配，大教室为 0 节时再在小教室中唯一匹配。

服务在 14:19:35 先用当前日期课表做一次只读会话预热；14:20 后最多 3 路并发读取周一至周六 6 个日期页。第一轮已发现的 L1 立即进入串行处理，同时后台在放课后第 2 / 6 / 10 秒以最多 2 路 GET 刷新全部日期；每个日期保留目标数不减少的最新页面，稳定快照补入后发布的同日 L1 / L1.5 / 软开，再继续后续课程层。课程卡资格和闻道规则由最多 2 个 worker 提前预检，完成超过 15 秒则在提交前重新预检。HTTPS 连接池最多保留 3 条 keep-alive 连接，整次服务上限 600 秒，以容纳六天动态规则可能发现的多节课程。真实 `do_addbook` 始终严格串行，任意时刻最多一个预约 / 候补 mutation；读取、发现和预检只在安全的只读路径并行。匹配字段为日期、实际课程名、课型 / 等级和起止时间；老师不参与 occurrence 幂等键，但工作日候选选择与提交排序使用老师优先层。工作日同名同时段先限定李俊（含空老师），未命中才选择其他老师；每个老师层内先限定大教室并要求恰好一节，大教室为 0 节时再限定小教室并要求恰好一节；周六保持不分老师、按教室优先。优先层出现多节时仍失败关闭，不猜测提交。`available` 执行预约；`queue_available` 只有在仓库配置显式设置 `allowWaitlist=true` 时执行候补；已预约或已排队不重复提交。网络波动、课程卡尚未开放、规则短暂未就绪或 mutation 明确返回 `NOTOPEN` 时，首次失败后最多重试 3 次，间隔为 80 / 160 / 320ms；明确已满、已停止、无卡或不可约时直接进入下一节。
