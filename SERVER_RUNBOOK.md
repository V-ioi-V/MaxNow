...
长期规则为：放课后扫描周一至周六全天课表，处理所有标准芭蕾 L1，以及课程名归一化后严格等于“软开”的课程；周日、软开专项、软开-胯和其他近似名称均排除。提交顺序先周六，再按周一至周五；同日按开始时间排序，同一时间软开在 L1 前。老师不限；每个同名同时段先在大教室中唯一匹配，大教室为 0 节时再在小教室中唯一匹配。
