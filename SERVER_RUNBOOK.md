...
长期规则为：放课后扫描周一至周六课表；周一至周五只处理 18:40（含）后开始的课，周六全天。课程范围为所有标准芭蕾 L1，以及课程名归一化后严格等于“软开”或“软开课”的课程；周日、软开专项、软开-胯和其他近似名称均排除。提交顺序先全部芭蕾 L1，再全部软开 / 软开课；每一课程层内先周六，再按周一至周五，同日按开始时间排序。老师不限；每个同名同时段先在大教室中唯一匹配，大教室为 0 节时再在小教室中唯一匹配。
