...
REQUEST_JSON=$(dd bs=10001 count=1 2>/dev/null)

if [ -z "$REQUEST_JSON" ] || [ "${#REQUEST_JSON}" -gt 10000 ]; then
  echo '{"source":"wenda-live","status":"configuration_error","live":false}' >&2
  exit 4
fi

printf '%s' "$REQUEST_JSON" | sudo -n systemd-run \
  --quiet \
  --wait \
  --pipe \
  --collect \
  ...
  /usr/bin/python3 -B scripts/cancel_ballet.py "$1"
