...
def check_ballet_booking_fast():
    ...
    required = (
        ...
        "OnFailure=maxnow-ballet-sync.service",
        "执行结束立即刷新芭蕾面板",
        "抢课后</dt><dd>执行结束立即刷新芭蕾面板",
        ...
        'id="ballet-booking-results-title">本地抢课结果',
    ...
    if any(marker not in combined for marker in required) or "ballet-booking-tab-panel" in dashboard_html:
...
def check_ballet_read_model():
    ...
    if (
        ...
        or '.topbar-data-status .status-pill:is([data-state="failed"], [data-state="error"])' not in dashboard_css
    ):
...
def check_secondary_view_style():
    ...
    if (
        ...
        or 'label.textContent = row.type === "gap"' not in dashboard_js
    ...
    weekly_font_root = ROOT / "dash/assets/fonts/long-cang"
    if (
        not template_path.is_file()
        or not brief_template_path.is_file()
        or not digits_manifest_path.is_file()
        or not (weekly_font_root / "LongCang-Weekly.woff2").is_file()
        or not (weekly_font_root / "LongCang-Regular.ttf").is_file()
    ...
    if (weekly_font_root / "LongCang-Weekly.woff2").stat().st_size > 100_000:
    ...
    if (
        "styles.css?v=248" not in dashboard_html
        or "styles.css?v=127" not in login_html
        or "app.js?v=209" not in dashboard_html
    ):
...
def check_ballet_growth_contract():
    ...
    if (
        ...
        or "--swan-lift: -21%;" not in dashboard_css
        or '.ballet-swan-stage[data-level="10"] {' not in dashboard_css
        or "--swan-lift: -1%;" not in dashboard_css
        or "transform: translateY(var(--swan-lift));" not in dashboard_css
        or "align-items: center;" not in dashboard_css.split(".ballet-growth-level-summary {", 1)[1].split("}", 1)[0]
...
def check_data_health_contract():
    ...
    if "app.js?v=209" not in dashboard_html:
