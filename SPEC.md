...
- 除 Home 外，豆奶、Token、芭蕾、云服务、生活和同行记统一使用 `secondary-view` / `secondary-page-head` 页面协议：共用轻色渐变白底、圆角、阴影、hover / focus 和状态 pill，不使用卡片顶部彩色横条；hover 只增强边框、阴影和轻微上浮，必须保留组件原有背景，不能统一洗成纯白；每页保留自己的语义色与内容结构，不为了视觉统一改写数据契约或交互。
...
- 下一节课：日期、周几、起止时间、课程、级别、老师、正式预约 / 候补状态、距离开课时间和可取消截止时间。
...
- 未来课程：芭蕾页最顶部使用两个同级独立 tab，左侧为标题 / 同步信息，右侧为紧凑“下一节预约”，宽桌面各占约 1/2 且同顶、同底、等高，不允许把右侧模块嵌套进左侧标题卡；右侧 tab 按“紧凑日期 / 弹性课程信息 / 状态”三段自适应分配宽度，标题与课程信息共用左基线，不使用固定宽度内容块强行居中；视口不超过 `1200px` 时两个 tab 自然堆叠，避免课程信息被半宽卡片压缩。独立“所有预约”列表同时完整展示所有尚未上课的已预约 / 排队中课程，包括顶部右侧的下一节，不显示已取消或已经结束的课程。列表的日期列上下展示日期与真实星期，课程列只保留时间和老师；“已预约”使用粉玫瑰色，“排队中”使用橙色，不能混成同一种状态色。后续按周扩展可预约、满员和已取消状态；剩余名额必须标记为抓取时快照，不能当作持续有效承诺。
