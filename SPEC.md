...
- 课表老师显示优先级补充：本条替代上一条允许中等桌面或重叠窄卡隐藏老师的旧规则。老师姓名在所有课程卡中都必须显示，包括 `1101px–1500px` 中等桌面、60 分钟紧凑卡和同教室重叠窄卡；课程名后优先保留独立老师行，人数 / 排队必要时压缩为不换行的单行，时间 / 状态也保持单行，不能通过隐藏或压缩老师换取空间。
- 课表时间显示优先级补充：起止时间属于不可省略的核心事实，任何桌面课程卡都不得用省略号或裁切隐藏结束时间。宽卡让完整时间与状态徽标同排且两者都不收缩；`1101px–1500px` 的 60 分钟紧凑卡空间不足时，状态徽标移到完整时间下一行，优先压缩人数 / 排队文字。
