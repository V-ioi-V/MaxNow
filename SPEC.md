...
- 今日状态：由前端根据今日 Todo、当前时段、ROADMAP 待推进 / 主线、Token 活跃和自动化状态自动推导“执行 / 推进 / 复盘 / 探索 / 巡检”等模式；右侧信号区使用青色 24 小时圆形进度环表达当天已过比例，百分比显示在环内，当前时间显示在环外，时段、推进、Token、自动化信号保留在右侧列表；`dashboard.json.today` 只作为当天人工 override，旧日期判断不再占据主状态。
