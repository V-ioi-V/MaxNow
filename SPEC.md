...
- 芭蕾页顶部提供小红书“芭蕾周记录”封面入口：固定使用 `1280×1710` 的粉色手作模板，中文标题和小写 `week` 作为固定视觉，`2026-07-27` 至 `2026-08-02` 为 `week 2`，按北京时间每周一进入下一周。运行时只在浏览器本地用 `dash/assets/ballet-week-cover/digits/` 的透明手绘数字拼出周数，不每周重新调用 AI，也不依赖系统字体；同周且模板版本未变化时复用当前页面缓存，生成结果支持预览、下载和在浏览器能力允许时复制到剪贴板。底图通过版本化模板文件和 `template.json` 更换，不要求修改页面脚本。
