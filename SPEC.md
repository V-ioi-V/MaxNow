...
- 本周课程表：放在芭蕾页最底部展示源站全部课程，不只展示 Owner 的预约。桌面卡片始终占满内容区可用宽度，横轴按固定一小时段分列、纵轴为星期与日期；课程按开始时间落入对应小时段，同一天同一小时段的多节课程在格内上下排列。最早与最晚课程之间，整周没有任何课程开始的连续小时必须合并为一个窄的 `xx:00–xx:00` 跳过列，不铺多个空白小时。`860px` 以下按日期分组并限制内部高度。桌面网格限制在约 `58vh` 内、可双向滚动且时间表头行 / 星期日期首列固定；滚动表头使用独立不透明遮罩和明确绘制层，不得与课程卡重叠。当天使用粉玫瑰描边；普通课程保持白底，按芭蕾、软开、肌肉素质、技术技巧和其他课型使用不同颜色描边；已预约使用粉玫瑰实心卡、已上完使用绿色实心卡、排队中使用橙色实心卡。过去但未上过的课程降低饱和度，已上完课程不得随过去日期一起淡化。通常展示当周周一至周日；周日 14:30 额外同步，只有确认下周至少存在一节课后才切换为“本周日 + 下周一至周日”共 8 天，并隐藏本周一至周六；未拿到下周数据时继续保留本周。
