...
- 核心指标：累计实际上课节数、累计分钟 / 小时、本月节数 / 小时和本周训练进度。时长的权威值始终保存为整数分钟，小时只在展示层换算，避免累计浮点误差；源记录无法确认时长时仍可计入节数，但不得猜测小时，并显示“有 N 节待补时长”。
...
- 课程分布：用水平条或紧凑排行展示所选时间段内的级别 / 课型分布；浏览器读取预聚合结果，不在每次切换时重新扫描全部历史。
...
- Owner 能看到 PHPSESSID 从原始实验起点到最后一次成功样本的已确认有效时长、最近 / 下次自动检查和 25 分钟间隔；页面不会把“持续活动仍有效”误写成“已经证明滑动续期”。
