...
- Home 主内容版式：状态条下方使用统一 `home-board` 两列外壳，`home-lane-primary` 承载左侧主任务，右侧 `home-side-stack` 承载所有次级信号和状态入口；`home-lane-signal` / `home-lane-rail` 只作为语义分组，视觉上展平成右侧同一条纵向流。所有 Home 模块都保留 `home-card-*` 和 `data-card-size`。不要再用固定 `grid-area`、局部左右列、固定高度或空白补丁拼模块，也不要为了三列对齐硬挤出窄栏。新增模块必须先在 `STYLE_CONTEXT.md` 的页面版式协议中确认语义 lane 和卡型。
