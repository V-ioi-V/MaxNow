...
- 本周训练：使用独立面板展示本周已完成、已预约、候补、预计节数和对应时长；预计采用“正式预约”为下限、“正式预约 + 候补”为上限，不能把候补直接算成一定会完成。宽桌面与课程卡并排，窄屏排在课程卡之前。
- 课程卡：使用独立面板展示卡名、开卡日期、开卡第几天、剩余 / 总课次、已用课次、有效期和同步时间，不内嵌进本周训练、预约或上课统计。宽桌面与本周训练并排，窄屏自然堆叠。每张卡独立使用自己的有效期和已用课次，不能用全局上课记录推断某张卡的消耗。每天以剩余课次 / 剩余有效周数计算到期前所需周频率，并展示“按建议整数周课次预计何时用完”和“若每周 1 节到期约剩多少”的计划情景。开卡未满 28 天时不展示实际节奏、能否用完或增量结论；满 28 天后才按该卡开卡以来的 `已用课次 / 实际开卡天数` 补充实际节奏预测。不得展示会员卡号、会员 ID 或源记录 ID。
...
- 未来课程：芭蕾页最顶部在低权重更新时间下方使用两个同级面板。桌面端左侧约占 1/3，仅展示下一节的日期、课程、状态、老师和取消截止；右侧约占 2/3，完整展示所有尚未上课的已预约 / 排队中课程，并包含左侧这节课。右侧各行保持统一，不再额外突出第一条；日期列上下展示日期与真实星期，“已预约”使用粉玫瑰色，“候补”使用橙色并在存在序号时显示“排队第 N 位”，不能只写笼统的“排队中”。`1200px` 以下两个面板单列堆叠且不得横向溢出。列表不显示已取消或已经结束的课程。后续按周扩展可预约、满员和已取消状态；剩余名额必须标记为抓取时快照，不能当作持续有效承诺。
