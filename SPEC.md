...
- 自动抢课先按日期优先级周六 → 周二 → 周五 → 周一 → 周三 → 周四，再在每天内部按 L1 → L1.5 → 软开 / 软开课排序；工作日同一天同课型先李俊（老师空白按李俊）、再其他老师，各组内按开始时间排序。周六不按老师分层；工作日同一日期、课程名与时间优先选择李俊老师层，未命中才选择其他老师层，每层教室仍按大教室优先、小教室兜底。周一至周五只处理 18:40（含）后开始的目标；周六按结束时间判断，只处理结束时间严格早于 18:00 的目标，18:00 整结束也排除；周日排除。周安排的准备抢卡必须原样展示“18:40 后 / 18:00 前结束”规则边界，旧数据里的周六“全天”也必须按现行规则显示为“18:00 前结束”。可预约则预约，可排队则候补；配置外课程一律跳过，不取消、转课、买卡、支付或登录。
