...
- 芭蕾页顶部同时保留低权重“选课指南”入口，登录后直接打开 Owner 保存的李俊芭蕾《课程类型与选课指南》完整原图，方便放大核对和另存。原图固定放在受认证保护的 `private-assets` 路径，不进入公开 `/assets/`、`ballet.*` 运行数据，也不自动修改课程等级、抢课目标或优先级。
...
- 抢课助手：使用四张等宽摘要卡，分别显示公开状态累计 `totalBooked` 的“累计抢到”、累计 `totalWaitlisted` 的“累计候补”、上次执行耗时和当前运行状态。耗时优先读取 `criticalPathMilliseconds`；旧记录缺少关键路径时回退所有正数 `elapsedMilliseconds` 之和，并在副说明保留有效目标数与逐目标平均值。
- 本地抢课结果：只显示总体统计，不展示逐课列表。总体统计固定为成功预约、候补中、未抢到三项，来源为 `lastRun.records` 的实际状态；标题旁显示记录数与最近执行时间。已预约 / 已在排队按对应成功类归组，其他未完成、未开放、唯一性失败或安全停止归入未抢到；存在公开候补位次时可在候补副说明中汇总。不得展示 timer、重试轨迹、Session、源记录 ID 或内部运行路径。
...
- 自动抢课按课程优先级 `芭蕾 L1 > 芭蕾 L1.5 > 软开 / 软开课` 排序。每一课程层内依次为周六全部课程、周一至周五李俊课程、周一至周五其他老师课程；老师字段为空按李俊处理，各组内部再按周一至周五与开始时间保持确定性。周六不按老师分层；工作日同一日期、课程名与时间优先选择李俊老师层，未命中才选择其他老师层，每层教室仍按大教室优先、小教室兜底。周一至周五只处理 18:40（含）后开始的目标，周六处理全天，周日排除。可预约则预约，可排队则候补；配置外课程一律跳过，不取消、转课、买卡、支付或登录。
