...
- 本周课程表：放在芭蕾页最底部展示源站全部课程，不只展示 Owner 的预约。`1501px` 以上宽桌面卡片左对齐占内容区约三分之二，`861px–1500px` 使用可用全宽以完整容纳 7 天，`860px` 以下按日期分组并限制内部高度。桌面网格限制在约 `58vh` 内、可双向滚动且日期行 / 时间列固定；滚动表头使用独立不透明遮罩和明确绘制层，不得与课程卡重叠。同一天、同一开始时间的多节课程在同一时间格内双列并排，超过两节时自然换行。当天使用粉玫瑰描边；普通课程保持白底，按芭蕾、软开、肌肉素质、技术技巧和其他课型使用不同颜色描边；已预约使用粉玫瑰实心卡、已上完使用绿色实心卡、排队中使用橙色实心卡。过去但未上过的课程降低饱和度，已上完课程不得随过去日期一起淡化。通常展示当周周一至周日；周日 14:30 额外同步，只有确认下周至少存在一节课后才切换为“本周日 + 下周一至周日”共 8 天，并隐藏本周一至周六；未拿到下周数据时继续保留本周。
