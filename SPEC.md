...
- 除 Home 外，豆奶、Token、芭蕾、云服务、生活和同行记统一使用 `secondary-view` 页面协议：共用轻色渐变白底、圆角、阴影、hover / focus 和状态 pill，不使用卡片顶部彩色横条；豆奶、Token、生活和同行记保留 `secondary-page-head`，芭蕾与云服务因顶部栏已有页面名称，不再重复标题卡。云服务进入内容区后直接展示“系统与托管”，芭蕾只在预约模块上方保留低权重更新时间。hover 只增强边框、阴影和轻微上浮，必须保留组件原有背景，不能统一洗成纯白；每页保留自己的语义色与内容结构，不为了视觉统一改写数据契约或交互。
...
- 页面页头：数据截至时间、最近同步结果和是否需要重新登录；正常状态与紧凑短时间一起收进全局顶部栏标题旁，不能在内容区单独占一行制造空白。完整年份时间通过 `title` 保留；异常告警仍紧随顶部栏出现在内容区。不展示 Cookie、Session 指纹、服务单元名或日志路径。
...
- 本周训练：与课程卡、成长进度组成页面顶部概览；`1501px` 以上三张卡各占内容区三分之一并保持同顶、同底、等高，`1101px–1500px` 本周训练与课程卡各占一半、成长进度独占下一行，`1100px` 以下单列。展示本周已完成、已预约、候补、预计节数和对应时长。预计采用“正式预约”为下限、“正式预约 + 候补”为上限，不能把候补直接算成一定会完成。
...
- 本周课程表：放在芭蕾页最底部展示源站全部课程，不只展示 Owner 的预约。桌面卡片始终占满内容区可用宽度，横轴以一小时段作为时间标尺、纵轴为星期与日期；每张课程卡的左右边界必须按实际起止分钟落点，`11:00–12:30` 应覆盖完整 `11:00–12:00` 和 `12:00–13:00` 的前半段，不能只占开始小时。同日时间重叠的课程分轨上下排列，不重叠课程可复用同一轨。最早与最晚课程之间，整周没有任何课程实际占用的连续小时必须合并为一个窄的 `xx:00–xx:00` 跳过列；跨过某小时的在上课程必须阻止该小时被压缩。桌面网格不设置内部固定高度或横纵滚动，全部小时列与 7 天课程直接在全宽面板内平铺，面板随内容自然增高且不得撑宽整页；`1100px` 以下按日期分组并同样自然展开，不设置内部滚动。当天使用粉玫瑰描边；普通课程保持白底，按芭蕾、软开、肌肉素质、技术技巧和其他课型使用不同颜色描边；已预约使用粉玫瑰实心卡、已上完使用绿色实心卡、排队中使用橙色实心卡。过去但未上过的课程降低饱和度，已上完课程不得随过去日期一起淡化。通常展示当周周一至周日；周日 14:30 额外同步，只有确认下周至少存在一节课后才切换为“本周日 + 下周一至周日”共 8 天，并隐藏本周一至周六；未拿到下周数据时继续保留本周。
- 训练记录：上课统计与上课历史合并为一张全宽面板，提供“本月 / 今年 / 全部”和“节数 / 时间”切换；主值、课程类型、课程级别和趋势必须同步采用同一时间范围与指标。只将闻道明确表示已上课 / 已完成的记录计入统计，历史保留日期、课程、级别、老师、起止时间和时长。所选范围少于 5 节时不展开大趋势图，改为说明“记录达到 5 节后显示趋势”，避免少量样本撑开整页。时长的权威值始终保存为整数分钟，小时只在展示层换算。
- 课程卡：放在页面顶部本周训练右侧并保持等宽；`1501px` 以上各占内容区三分之一，`1101px–1500px` 各占一半，`1100px` 以下随概览退回单列。卡内按卡信息、课程使用 / 有效进度、计划结论纵向收纳。每张卡独立使用自己的有效期和已用课次，不能用全局上课记录推断某张卡的消耗。开卡未满 28 天时不展示实际节奏、能否用完或增量结论；满 28 天后才按该卡开卡以来的 `已用课次 / 实际开卡天数` 补充实际节奏预测。不得展示会员卡号、会员 ID 或源记录 ID。
- 成长进度：放在顶部概览第三格，规则语义以 private personal-wiki 的 `raw/relationship-ricky/docs/2026-07-20-lijun-ballet-course-guide.md` 为来源。课程路径固定为 `L1 → L1.5 → L2 → L3 → L4 → L5`；L1.5、L2、L3 分别要求至少 1、3、5 个月芭蕾基础。升班分在存在明确月份要求时由当前级别课次 70 分与累计芭蕾基础时长 30 分组成；L4 / L5 这类来源只写“上一级基础”的阶段以当前级别课次形成 100 分参考。规律口径要求至少连续 21 天样本且最近 28 天达到每周 2 节，否则使用成人学习建议表的间歇课次阈值。达到 100 分只提示咨询老师测评，不自动宣称已具备升班资格。
- 经验等级：全部实际上课记录参与 XP，芭蕾基训每节 100 XP、技术技巧 80 XP、不定期开设课程 / 其他 70 XP、肌肉素质 60 XP、软开 50 XP。经验等级的六个名称与效果对应课程指南的课程定位：发力启蒙 / 正确发力方向、单一元素 / 动作标准、枢纽建立 / 重心与骨盆、协调开范 / 四肢协调、组合流畅 / 舒展与标准、认知串联 / 控制与完整。XP 是 MaxNow 游戏化进度，不是机构评分、健康评价或老师考核结论。

<!-- BALLET_GROWTH_CONTRACT_START -->
#### 成长评分协议（代码与文档必须同步）

`SPEC.md` 是成长评分机制的唯一人类可读协议，执行代码位于 `dash/app.js` 的 `BALLET_PROMOTION_RULES`、`BALLET_XP_BY_COURSE_TYPE`、`BALLET_XP_LEVELS` 与对应计算函数。任何课程路径、课次阈值、月份要求、评分权重、规律判定、XP 权重、等级阈值、等级名称或效果文案变更，都必须在同一分支同时修改代码、本节协议和 `UPDATE_LOG.md`；`scripts/check.py` 必须校验下列表格与代码常量一致，禁止只改一边。

升班分只读取脱敏 read model 中未取消、未缺席的实际上课记录。当前级别取已完成芭蕾基训记录中出现的最高级别；当前级别课次只统计该级别的芭蕾基训。观察窗口以数据截至日为锚点，当前级别首条记录至锚点至少覆盖 21 天，且最近最多 28 天的平均频率达到每周 2 节，才使用“规律课次”；否则使用“间歇课次”。基础月份从全部有效芭蕾基训的首条记录起算，每月按 30.4375 天换算。

存在明确基础月份要求时：

`升班分 = round(min(当前级别课次 / 目标课次, 1) × 70 + min(累计基础月份 / 要求月份, 1) × 30)`

没有明确月份数字、只要求“上一级基础”时：

`升班分 = round(min(当前级别课次 / 目标课次, 1) × 100)`
...
全部未取消、未缺席的实际上课记录参与 XP；同一节课只按归一化后的 `courseType` 计一次。下列英文标识是 read model 的执行键，中文名称是 Owner 可读含义：
...
页面上的 `6 / 100 分`、`150 / 300 XP` 等数值属于随上课记录变化的运行结果，不写死在产品规范中。达到 100 分只能显示“可咨询老师进行下一等级测评”；等级与效果也只表达 MaxNow 内部游戏化进度，不能替代老师判断。
...
- 学习趋势：训练记录达到 5 节后，在同一面板内展示折线趋势，提供“月 / 年 / 全部”周期和“节数 / 小时”指标切换，不使用双 Y 轴。月视图按日、年视图按月、全部视图按月或年展示，缺失月份补零，图例和单位随指标切换。
...
- 预约自动化：芭蕾页的课程计划只展示目标课程和逐课业务结果；Cloud 独立卡展示 `off` / `dry-run` / `enabled`、固定优先级、上次 / 下次执行、累计成功数、重试与失败边界。Session 实验详情也只在 Cloud 展示。
...
- 另用 `/var/lib/maxnow-ballet/sync-state.json` 保存每次同步尝试结果；失败只能原子更新这个状态文件，不能改成功账本。未来预约、课程卡与周课表分别使用 `booking-snapshot.json`、`membership-snapshot.json` 和 `timetable-snapshot.json`，都不混入 attendance ledger。课程表快照只保存日期、时间、课程、类型 / 级别、老师、场地、人数快照与可约状态，不保存源记录 ID 或原始 HTML。
- 逻辑日以 `Asia/Shanghai` 的 00:00 为界，生产 rolling 任务每天 00:00 运行；同一任务在周日 14:30 再运行一次以读取已发布的下周课表，并与 14:20 自动抢课关键窗口错开。任务使用单实例锁、有限网络重试；身份失效不重试。
- 每日 00:00 任务同时刷新当前预约、候补、上课记录、课程卡和周课表；预约默认 TTL 为 36 小时，成功后记录 `dataAsOf`，失败继续保留上一份并按年龄标记 stale。每月 1 日 00:47 执行全量校验。后续受控约课任务若真实改变预约，必须在结束后刷新该快照；页面打开本身永远不刷新。
...
- 采集器只允许已确认的只读 GET 页面，包括严格日期格式的课程表路径；禁止调用预约、取消、候补、转课、登录写接口或未知方法。日常同步为北京时间 00:00，周日 14:30 额外同步，月度 full 保持每月 1 日 00:47。
