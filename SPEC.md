...
- 未来课程：芭蕾页最顶部使用两个同级独立 tab，左侧为标题 / 同步信息，右侧为紧凑“下一节预约”，桌面端各占约 1/2 且同顶、同底、等高，不允许把右侧模块嵌套进左侧标题卡；`860px` 以下两个 tab 自然堆叠。独立“所有预约”列表同时完整展示所有尚未上课的已预约 / 排队中课程，包括顶部右侧的下一节，不显示已取消或已经结束的课程。列表的日期列上下展示日期与真实星期，课程列只保留时间和老师；“已预约”使用粉玫瑰色，“排队中”使用橙色，不能混成同一种状态色。后续按周扩展可预约、满员和已取消状态；剩余名额必须标记为抓取时快照，不能当作持续有效承诺。
