...
- 三周周安排：周期固定为周一至周日，用户只能在上周 / 本周 / 下周三档之间切换。上周和本周优先从实际完成记录、当前预约 / 候补快照以及脱敏课表中的 Owner 状态合并去重；相同日期、起止时间和标准课型的“芭蕾 L1 / 芭蕾L1-入门”或“软开 / 软开课”必须视为同一课程，预约快照和课表事实优先于 Fast Path 的泛化结果，不能把 9 节重复显示成 18 节。实际课程使用与下方完整课程表一致的左侧时间轴、长空档压缩和分钟定位；压缩空档左侧只显示空档起始整点（例如 `13:00`），不额外显示 `13:00–18:00` 范围文案，也不在压缩带下沿重复显示 `18:00`。时间轴底部收在全周最后一节课的准确结束时间，例如最后一节 `21:15` 结束时不再继续留白到 `22:00`。课程块复用课型色、预约 / 候补深浅、老师 / 教室、完整起止时间与状态结构。下周尚无同周 Fast Path 结果时，因为没有可信具体课时，按公开目标规则显示“准备抢”的课程与日期，并用小字号标出真实执行优先级。有同周 `lastRun.records` 后，以逐课真实课程、时间与 booked / waitlist / failure 状态替换计划；不得用滚动后的新目标回填旧结果。周安排不展示源站全部可约课程，不展示 Session、源 ID、内部路径或原始响应。桌面七天直接平铺，`860px` 以下允许周安排内部横向浏览，页面自身仍不得溢出。
