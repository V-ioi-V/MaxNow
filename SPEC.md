...
- 课程预约：摘要只展示从浏览器当天开始、最近三个“实际有预约 / 候补的日期”中的全部课程，而不是简单截取前三节；同一天连续多节必须完整保留，更远日期不进入摘要。首条未来预约继续使用粉白整行高亮和“最近一节”标记；日期列上下展示日期与真实星期，“已预约”使用粉玫瑰色，“候补”使用橙色并在存在序号时显示“排队第 N 位”。预约行保留日期、时间、课程、老师、教室、取消截止和业务状态。
- 三周周安排：周期固定为周一至周日，用户只能在上周 / 本周 / 下周三档之间切换。上周和本周优先从实际完成记录、当前预约 / 候补快照以及脱敏课表中的 Owner 状态合并去重；下周尚无同周 Fast Path 结果时，按公开目标规则显示“准备抢”的课程与日期，并用小字号标出真实执行优先级。有同周 `lastRun.records` 后，以逐课真实课程、时间与 booked / waitlist / failure 状态替换计划；不得用滚动后的新目标回填旧结果。周安排不展示源站全部可约课程，不展示 Session、源 ID、内部路径或原始响应。桌面七天直接平铺，`860px` 以下允许周安排内部横向浏览，页面自身仍不得溢出。
