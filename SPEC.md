...
- 课程计划：左侧“课程预约”展示当前正式预约 / 候补，右侧“抢课”集中承载自动抢课。预约列表首条未来预约使用粉白整行高亮和“最近一节”标记；日期列上下展示日期与真实星期，“已预约”使用粉玫瑰色，“候补”使用橙色并在存在序号时显示“排队第 N 位”。抢课区顶部使用两张摘要卡：第一张同时显示公开状态累计 `totalBooked` 的“已抢到”和当前未来预约 / 候补列表数量的“已预约”，第二张显示上次执行记录中所有正数 `elapsedMilliseconds` 的四舍五入平均值，无有效耗时时显示 `-- ms`。摘要下方通过“代抢 / 上次抢课结果”两个 Tab 互斥展示本轮目标与最近一次执行记录；结果必须使用上次记录自身的日期和课程字段，不能用滚动后的新目标回填。正式预约 / 候补行和抢课行都展示日期、时间、课程、老师、教室与结果，不展示 timer、重试、Session 或内部运行路径。桌面左右两列，`860px` 以下单列，`560px` 以下摘要卡与列表行自然堆叠且不得横向溢出。
