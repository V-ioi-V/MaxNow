...
- 周简报使用另一张版本化布艺底图；“芭蕾周记录”、“第 / 周”和六个指标名称作为固定底图文字，只动态填写周数、日期、本周训练次数 / 时长 / 最爱课程、截至该周的总次数 / 总时长 / 最爱课程。动态字统一使用项目内置的 Ma Shan Zheng 字体，不依赖访问设备安装字体。周简报固定取最近一个已经到达的北京时间周日 `20:00` 为截止点，六项完全复用训练概览的已完成记录、时长与最爱课程口径；周日 `20:00` 增加一次 rolling 只读同步以提供该截止点数据，后续同步产生的新课程会被截止时间过滤。源数据尚未达到该截止点时，弹窗必须明确显示实际数据暂截至时间。
- 进入芭蕾页后利用浏览器空闲时间预生成默认 `week N` 封面；周简报只在切换到该页后生成，不得同时拉取两张底图。运行时底图使用轻量 WebP，动态文字使用同一 `Ma Shan Zheng` 字体的 WOFF2 网页子集；同周、同模板版本且源数据未变化时复用当前页面缓存。底图、位置与版本统一通过 `dash/assets/ballet-week-cover/template.json` 管理；生成只发生在浏览器，不上传或在服务器保存 PNG 成品。
