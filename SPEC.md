...
- 周简报使用另一张版本化布艺底图；“芭蕾周记录”、“第 / 周”和六个指标名称作为固定底图文字，只动态填写周数、`MM.DD–MM.DD` 纯数字日期、本周训练次数 / 时长 / 最爱课程、截至该周的总次数 / 总时长 / 最爱课程；动态周数必须与底图“第 / 周”基线视觉对齐。动态字固定使用项目内置的 Long Cang（龙藏体）字体，并在画布合成时增加同色轻量圆润描边；它的宽松手写结构用于贴近固定标题，并作为后续每周不再随意更换的统一字体，不依赖访问设备安装字体。每个周一至周日课程周期以该周期最后一节已预约或实际上课课程的结束时间为统计截止点；结束后 `10` 分钟执行一次完整只读芭蕾刷新，再过 `10` 分钟（结束后 `20` 分钟）生成周简报。六项完全复用训练概览的已完成记录、时长与最爱课程口径；源数据未达到收尾刷新时间时不得用旧数据生成本周期简报。
- 进入芭蕾页后利用浏览器空闲时间预生成默认 `week N` 封面；周简报到达本周期生成时间且收尾数据已经刷新时自动生成，页面当时未打开则在下次进入后立即生成。运行时底图使用轻量 WebP，动态文字使用同一 `Long Cang` 字体的 WOFF2 网页子集；同周、同模板版本且源数据未变化时复用当前页面缓存。底图、位置与版本统一通过 `dash/assets/ballet-week-cover/template.json` 管理；生成只发生在浏览器，不上传或在服务器保存 PNG 成品。
