...
- 最近一节课：不再单设重复面板；在课程计划的预约列表中，将当前时刻之后最近的一节预约整行高亮并标记“最近一节”，当天已经开始或结束但仍保留在三日摘要中的课程不得抢占该标记。该行继续展示日期、周几、起止时间、课程、级别、老师、教室、正式预约 / 候补状态和可取消截止时间。源文案为“课前 N 小时可取消”时，必须以开课时间减去 N 小时计算绝对截止时间；无法可靠解析时保留并显示源文案，不能拼出不完整的“可取消至 前可取消”。
