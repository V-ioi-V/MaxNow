...
- 除 Home 外，豆奶、Token、芭蕾、云服务、生活和同行记统一使用 `secondary-view` 页面协议：共用轻色渐变白底、圆角、阴影、hover / focus 和状态 pill，不使用卡片顶部彩色横条；豆奶、Token、云服务、生活和同行记保留 `secondary-page-head`，芭蕾因顶部栏已有页面名称，不再重复标题卡，只在预约模块上方保留低权重更新时间。hover 只增强边框、阴影和轻微上浮，必须保留组件原有背景，不能统一洗成纯白；每页保留自己的语义色与内容结构，不为了视觉统一改写数据契约或交互。
