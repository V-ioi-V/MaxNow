...
- 训练记录：作为芭蕾页最后一个模块，上课统计与上课历史合并为一张全宽面板，提供“本月 / 今年 / 全部”和“节数 / 时间”切换；主值、课程类型、课程级别、授课老师、图表与历史必须同步采用同一时间范围与指标。本月使用按日历日期排布的热力图，有已加载的训练历史后即可展示；热力图外框只按七列日历所需宽度紧凑展开并左对齐，不得为了占满全宽留下大块横向空白，未来尚未纳入同步的日期使用低对比浅底虚线格，不铺密集斜纹或重复破折号。今年使用从 1 月到浏览器当前月的按月折线，不显示未来月份；全部使用仅包含实际有记录年份的年度折线。两档折线图也必须按实际时间点数量收敛并左对齐，桌面最大宽度与热力图一致为 `840px`，数据少时进一步缩短，手机端才占满可用宽度。只要所选范围存在上课记录就展示对应图表；没有记录时显示明确空状态。桌面端图表左置、紧凑历史右置，右侧通过脱离网格行高计算的铺满层始终与左侧图表同高，只预览最近 5 节；右上角显示当前范围总节数，底部固定“查看全部上课记录”。完整历史必须在独立右侧抽屉中滚动，不允许在原位置加载更多或随记录增长撑高主页面。`1100px` 以下改为图表在上、历史在下的自然布局，手机端只预览最近 3 节。课程级别有明确等级时显示 L1 / L1.5 等等级，没有等级时不显示“无级别”，改用软开、肌肉素质、技术技巧等真实课程类型。授课老师作为与课程类型、课程级别并列的独立分布卡，按所选节数或时间显示各老师的课程占比。只将闻道明确表示已上课 / 已完成的记录计入统计，历史保留日期、课程、展示级别 / 课型、老师、起止时间和时长。时长的权威值始终保存为整数分钟，小时只在展示层换算。
