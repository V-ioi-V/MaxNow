...
- 成长等级：全部实际上课记录每节固定计 1 节，不按课程类型折算分数或 XP。规则只使用 `Lv.1–Lv.10`，累计 200 节进入 Lv.10 满级；为避免与课程 `L1 / L1.5` 混淆，页面把它显示为“第 1–10 阶段”，使用十个编号节点和十张本地透明 PNG 模拟小天鹅从灰色绒毛雏鹅逐渐长成白色成年天鹅的过程，不增加等级名称、技术效果或评分含义。成长卡顶部不重复显示 `Lv.N`。
