...
- 课程计划：宽桌面左侧“课程预约”占三分之一并纵向贯穿模块；右侧占三分之二，顶部并排“抢课助手 / 本地抢课结果”，下方承载“周安排”，避免周安排为了追平预约列表被拉出大面积空白。顶部两区外框与内部统计卡必须同顶、同底、同高，并压缩统计卡下方无意义留白。`1320px` 以下先显示预约，再并排助手 / 结果，最后显示周安排；`860px` 以下四区按同一顺序全部单列且不得造成整页横向溢出。预约存在明确取消截止时显示绝对时间；当天课程已经退出可取消预约快照、只剩已预约课表状态时，显示“已过可取消时段”或“课程已结束”，不得伪造具体截止时间。
