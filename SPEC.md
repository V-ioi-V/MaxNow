...
- 逻辑日仍以 `Asia/Shanghai` 的 00:00 为界；生产 rolling 任务每天 `09:00 / 12:00 / 15:00 / 18:00 / 22:00` 运行，并在周日 14:30 额外读取抢课后发布的下周课表、周日 20:00 为训练周简报刷新脱敏数据。任务使用单实例锁、有限网络重试；身份失效不重试。
