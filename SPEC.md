...
- 课程表分组层级：桌面日期边界与同日教室边界都使用自然的 `1px` 细线，不画贯穿全高的粗线；日期线使用稍深的暖灰粉，教室线使用接近背景的浅灰粉，通过色阶而非粗细形成层级。移动端继续用独立日期卡片分组。完整课程表恢复原有紧凑时间刻度，60 分钟及以下课程使用缩小的课程名、说明、时间和紧凑内边距完整承载信息；90 分钟等长课仍按真实时长明显拉开，长空档继续单独压缩。
