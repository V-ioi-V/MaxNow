...
- 课表老师显示优先级补充：本条替代上一条允许中等桌面或重叠窄卡隐藏老师的旧规则。老师姓名在所有课程卡中都必须显示，包括 `1101px–1500px` 中等桌面、不足 60 分钟的紧凑卡和同教室重叠窄卡；课程名后优先保留独立老师行，人数 / 排队必要时压缩为不换行的单行，时间 / 状态也保持单行，不能通过隐藏或压缩老师换取空间。
- 课表时间显示优先级补充：起止时间属于不可省略的核心事实，任何桌面课程卡都不得用省略号或裁切隐藏结束时间。宽卡让完整时间与状态徽标同排且两者都不收缩；`1101px–1500px` 的不足 60 分钟紧凑卡空间不足时，状态徽标移到完整时间下一行，优先压缩人数 / 排队文字。
- 软开课色相补充：原“灰米色”统一解释并实现为偏奶咖的暖米色，不再使用中性或偏冷灰；普通、排队、预约和已上完仍只改变这一暖色色相的明度与深度。
- 课程表分组层级：桌面日期边界与同日教室边界都使用自然的 `1px` 细线，不画贯穿全高的粗线；日期线使用稍深的暖灰粉，教室线使用接近背景的浅灰粉，通过色阶而非粗细形成层级。移动端继续用独立日期卡片分组。完整 60 分钟课程使用标准卡片字号、内边距和信息层级，只有不足 60 分钟的短课才进入紧凑模式；宽屏分钟刻度必须为一小时课程保留足够内容高度，长空档继续单独压缩，避免一小时课程看起来像被挤进时间缝隙。
