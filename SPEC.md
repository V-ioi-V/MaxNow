...
- 课程表分组层级：桌面日期边界与同日教室边界都使用自然的 `1px` 细线，不画贯穿全高的粗线；日期线使用稍深的暖灰粉，教室线使用接近背景的浅灰粉，通过色阶而非粗细形成层级。移动端继续用独立日期卡片分组。完整课程表恢复原有紧凑时间刻度，60 分钟及以下课程统一复用紧凑卡规则，仅将“报名 / 容量 / 全班排队人数”行和完整起止时间缩至 `6.5px`；课程名、老师和可约、可排队、已满、已预约等状态标签保持与同轨 90 分钟课程一致的字号和标签尺寸。所有状态都必须位于完整时间下方，在卡片最底部左对齐独占一行，不能按状态类型局部覆盖。90 分钟等长课仍按真实时长明显拉开，长空档继续单独压缩。
