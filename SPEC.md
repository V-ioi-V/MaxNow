...
3. Token
   - Token 使用详情页。
4. 云服务
   - 服务器自动化详情页，只读列出云服务器上的定时任务、数据同步、站点托管、系统状态和运行边界；不保留与下方任务卡重复的顶部摘要卡。
5. 生活
...
- 时间点：日程、截止时间、自动化运行时间。
- 系统状态：作为云服务页入口，用来快速判断机器是否健康；点击卡片进入“云服务”页查看 Host、nginx、证书、部署版本、CPU、磁盘、内存和运行时间等完整服务器状态快照。
...
- 最新 AI 信号：优先显示当天捕捉到的 1-5 条高相关 AI 外部变化；如果当天暂无新条目，则回退显示最近 7 天内最新的高相关信号，避免每日 00:00 刷新后空白。
