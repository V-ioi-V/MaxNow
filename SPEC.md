...
- 课程计划：左侧“课程预约”展示当前正式预约 / 候补，右侧“抢课”集中承载自动抢课。预约列表首条未来预约使用粉白整行高亮和“最近一节”标记；日期列上下展示日期与真实星期，“已预约”使用粉玫瑰色，“候补”使用橙色并在存在序号时显示“排队第 N 位”。抢课区顶部使用三张等宽摘要卡，分别显示公开状态累计 `totalBooked` 的“累计抢到”、累计 `totalWaitlisted` 的“累计候补”，以及上次执行 `criticalPathMilliseconds` 的“上次抢课耗时”；旧记录缺少关键路径时回退为所有正数 `elapsedMilliseconds` 之和。耗时以易读的 `ms / s` 显示，并在副说明给出有效目标数与逐目标平均值，避免把 `66,107 ms` 误读成 `66 ms`。摘要下方通过“代抢 / 上次抢课结果”两个 Tab 互斥展示本轮目标与最近一次执行记录；结果必须使用上次记录自身的日期和课程字段，不能用滚动后的新目标回填。正式预约 / 候补行和抢课行都展示日期、时间、课程、老师、教室与结果，不展示 timer、重试、Session 或内部运行路径。同一结果列表中的课程名称必须共用固定左边缘，不得因右侧状态徽标宽度不同而左右漂移。桌面左右两列，`860px` 以下单列；三个摘要卡在桌面与手机端都保持等分，列表行在 `560px` 以下自然堆叠且不得横向溢出。
