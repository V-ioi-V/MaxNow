...
- 今日状态：由前端根据今日 Todo、当前时段、ROADMAP 待推进 / 主线、Token 活跃和自动化状态自动推导“执行 / 推进 / 复盘 / 探索 / 巡检”等模式；右侧信号区使用上方 24:00、下方 00:00 的今日进度轴表达当天已过时间，当前时间由下向上推进，时段、推进、Token、自动化信号作为节点挂在轴旁；`dashboard.json.today` 只作为当天人工 override，旧日期判断不再占据主状态。
...
- AI 前沿：Home 只保留一个 AI 前沿简报模块，由 Last-30 展示“最新发布 / 本周前沿 / 近 30 天关键进展”。标题和摘要必须以中文事实为主，品牌、模型名和 API 名可保留英文；不要再单独铺一张重复的 AI 外部输入卡，也不承接杂项链接。
