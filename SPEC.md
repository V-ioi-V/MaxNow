...
- 自动抢课按课程优先级 `芭蕾 L1 > 芭蕾 L1.5 > 软开 / 软开课` 排序。每一课程层内依次为周六符合时段的课程、周一至周五李俊课程、周一至周五其他老师课程；老师字段为空按李俊处理，各组内部再按周一至周五与开始时间保持确定性。周六不按老师分层；工作日同一日期、课程名与时间优先选择李俊老师层，未命中才选择其他老师层，每层教室仍按大教室优先、小教室兜底。周一至周五只处理 18:40（含）后开始的目标，周六只处理 18:00 前开始的目标，周日排除。可预约则预约，可排队则候补；配置外课程一律跳过，不取消、转课、买卡、支付或登录。
