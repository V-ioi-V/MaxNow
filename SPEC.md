...
- 课程预约：摘要只展示从浏览器当天开始、最近三个“实际有预约 / 候补的日期”中的全部课程，而不是简单截取前三节；当天按完整自然日纳入，不能因某节课已经开始就从摘要中删除。同一天连续多节必须完整保留，更远日期不进入摘要。摘要以日期为外层分组：同一天只渲染一个大卡，左侧统一显示 `M.D` 与真实星期，右侧按时间顺序纵向放置当天每节课程的独立小卡；窄屏把日期移到顶部，课程小卡仍依次排列。摘要以当前预约 / 候补快照为主，并用脱敏课表中明确为 Owner 已预约 / 排队的课程补齐当天已开始但不再位于 upcoming 的记录；两源按日期、起止时间和标准课型语义去重。“已预约”使用粉玫瑰色，“候补”使用橙色并在存在序号时显示“排队第 N 位”。每张课程小卡保留时间、课程、老师、教室、取消截止和业务状态。
