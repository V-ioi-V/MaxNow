...
- 未来课程：最近一节继续进入“下一节预约”主卡；独立“所有预约”列表同时完整展示所有尚未上课的已预约 / 排队中课程，包括主卡中的下一节，不显示已取消或已经结束的课程。列表显示日期、时间、课程、老师、级别和预约状态，不与实际上课历史混排。后续按周扩展可预约、满员和已取消状态；剩余名额必须标记为抓取时快照，不能当作持续有效承诺。
