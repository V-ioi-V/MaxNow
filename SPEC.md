...
- 芭蕾页后续提供小红书“芭蕾课周记录”封面入口：固定使用 `1280×1710` 的粉色手作模板，中文标题和小写 `week` 作为固定视觉，周数从 2026-08-02 所在周的 `week 2` 起按北京时间每逢周日加一。运行时只在浏览器本地用 `dash/assets/ballet-week-cover/digits/` 的透明手绘数字拼出周数，不每周重新调用 AI，也不依赖系统字体；生成结果支持预览、下载和在浏览器能力允许时复制到剪贴板。
