...
不要在豆奶页面加入签到操作、账号登录、豆丁展示或 cron 管理。豆丁只作为原始数据保留。账号余量只能从 OpenClaw / 服务器侧登录态生成的数据快照读取，前端不直接访问豆奶站点。
...
`dash/data/last-30.json` 负责今日大事、本周大事、近 30 天主线、重要决定和等待项。
...
`dash/data/dounai_checkin.json` 负责豆奶每日签到记录、账号余量快照和账号日均可用历史，由 OpenClaw 签到自动化更新；前端只读取流量、账号有效期延长时长、累计签到天数、近 30 天记录、剩余可用流量、有效期、每日可用预算和 `account_history`，不编辑、不回写，也不修改签到脚本或 cron。
