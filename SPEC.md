...
- 未来课程：芭蕾页最顶部只保留低权重更新时间和一张全宽“所有预约”面板，完整展示所有尚未上课的已预约 / 排队中课程，不再用独立卡重复下一节。列表第一条就是下一节，并以低权重“下一节”标识和浅粉背景突出，同时保留该课的取消截止时间；后续条目只显示时间和老师。日期列上下展示日期与真实星期；“已预约”使用粉玫瑰色，“候补”使用橙色并在存在序号时显示“排队第 N 位”，不能只写笼统的“排队中”。列表不显示已取消或已经结束的课程。后续按周扩展可预约、满员和已取消状态；剩余名额必须标记为抓取时快照，不能当作持续有效承诺。
