...
- 课表时间显示优先级补充：起止时间属于不可省略的核心事实，任何桌面课程卡都不得用省略号或裁切隐藏结束时间。宽卡让完整时间与状态徽标同排且两者都不收缩；`1101px–1500px` 的 60 分钟及以下紧凑卡空间不足时，状态徽标移到完整时间下一行，优先压缩人数 / 排队文字。
- 软开课色相补充：原“灰米色”统一解释并实现为偏奶咖的暖米色，不再使用中性或偏冷灰；普通、排队、预约和已上完仍只改变这一暖色色相的明度与深度。
- 课程表分组层级：桌面日期边界与同日教室边界都使用自然的 `1px` 细线，不画贯穿全高的粗线；日期线使用稍深的暖灰粉，教室线使用接近背景的浅灰粉，通过色阶而非粗细形成层级。移动端继续用独立日期卡片分组。完整课程表恢复原有紧凑时间刻度，60 分钟及以下课程使用 `9px` 课程名和 `7px` 说明 / 时间及紧凑内边距完整承载信息；正式预约状态必须位于完整时间下方，在卡片最底部左对齐独占一行。90 分钟等长课仍按真实时长明显拉开，长空档继续单独压缩。
