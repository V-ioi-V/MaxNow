...
5. 同步脚本
   ...
   - `scripts/sync_ballet.py` 只通过已确认的闻道只读页面读取预约与实际上课记录；上课历史首屏不足累计数时，仅允许调用官方页面声明的固定 `newcheckrecord/{storeId}/{offset}` 分页 POST，并用私有账本按“日期 + 课程名”唯一补齐摘要记录。活动预约同样校验官方预约页声明的总数、`customerid` 和固定 `newbookrecord/{storeId}/{offset}` 分页 POST，读取全部列表页并按源记录 ID 合并去重后，才筛选预约 / 候补并读取详情；同步、实时查询和 Fast Path 复核共用该读取器。分页 contract 变化、无进展或事实冲突时失败关闭；同步不包含预约、取消、候补或转课写操作。
...
Dashboard 启动时必须先按 URL hash 显示目标页，再只读取该页所需数据：Home 读取首页摘要源，芭蕾只读取课程、Session 状态和 Fast Path 结果，Cloud 只读取系统与芭蕾运维源，Token、豆奶、生活和同行记各自读取独立数据。60 秒内切页复用已完成请求，超过窗口后按现有 HTTP 缓存规则重新校验；手动刷新和 5 分钟自动刷新只刷新当前页。不得为了更新隐藏页面让二级页等待整套 Home 数据。
...
- 周安排时间轴标签以本条为准：压缩空档只保留斜纹提示，不显示空档起始整点；底部仍按最后一节课的实际结束分钟精准收口，但不再重复显示该结束时间。课程卡内完整起止时间与普通整点刻度不受影响。
- 训练记录的时间范围初次打开默认选择“全部”；“本月 / 今年”仅在用户主动切换后生效，缺失或无效范围同样回退到“全部”。
- 训练记录“全部”范围以本条为准：旧年度折线改为按周一至周日聚合的热力图，按实际有记录的年份分组，从首个训练周补齐到最后训练周或最近成功同步周；每格直接显示周起始日期与节数 / 小时，桌面最大 `840px`，手机重排周格且不得产生横向溢出。本月日历热力图和今年月度折线保持不变。
...
- 芭蕾页顶部在“已同步”状态右侧提供小红书“芭蕾周记录”入口；`week N` 继承同一状态胶囊的字号、字重和高度。弹窗内用双页横向滑动区承载 `week N` 封面与周简报，也提供两个明确切换按钮；当前页的复制 / 下载操作始终作用于正在显示的图片。两张输出都固定为 `1280×1710`，支持预览、下载和在浏览器能力允许时复制到剪贴板。
- `week N` 封面继续使用粉色手作模板，中文标题和小写 `week` 作为固定视觉；`2026-07-27` 至 `2026-08-02` 为 `week 2`，按北京时间每周一进入下一周。运行时只在浏览器本地用 `dash/assets/ballet-week-cover/digits/` 的透明手绘数字拼出周数，不每周重新调用 AI，也不依赖系统字体。
- 周简报使用另一张版本化布艺底图；“芭蕾周记录”、“第 / 周”和六个指标名称作为固定底图文字，只动态填写周数、`MM.DD–MM.DD` 纯数字日期、本周训练次数 / 时长 / 最爱课程、截至该周的总次数 / 总时长 / 最爱课程；动态周数必须与底图“第 / 周”基线视觉对齐。动态数字统一使用无描边、常规字重的系统 UI 字体，避免毛笔数字过粗或变形；动态课程名继续使用项目内置 Ma Shan Zheng，并保留同色轻量圆润描边，使课程名与布艺底图协调。每个周一至周日课程周期以该周期最后一节已预约或实际上课课程的结束时间为统计截止点；结束后 `10` 分钟执行一次完整只读芭蕾刷新，再过 `10` 分钟（结束后 `20` 分钟）生成周简报。六项完全复用训练概览的已完成记录、时长与最爱课程口径；源数据未达到收尾刷新时间时不得用旧数据生成本周期简报。
- 进入芭蕾页后利用浏览器空闲时间预生成默认 `week N` 封面；周简报到达本周期生成时间且收尾数据已经刷新时自动生成，页面当时未打开则在下次进入后立即生成。运行时底图使用轻量 WebP，动态课程名使用 `Ma Shan Zheng` WOFF2 网页子集，数字使用设备 UI 字体栈；同周、同模板版本且源数据未变化时复用当前页面缓存。底图、位置与版本统一通过 `dash/assets/ballet-week-cover/template.json` 管理；生成只发生在浏览器，不上传或在服务器保存 PNG 成品。
...
- 多卡课程票券必须填满课程卡所在的概览格；上一张 / 下一张按钮、圆点和页码固定在票券底部内侧，不在票券外另占一行，并预留卡内空间避免覆盖计划结论。
...
- 最近一节课：不再单设重复面板，也不渲染“最近一节”文字标签；在课程计划的预约列表中，将当前时刻之后最近的一节预约整张课程小卡使用更深一级的浅粉底色高亮，当天已经开始或结束但仍保留在三日摘要中的课程不得抢占该高亮。该行继续展示日期、周几、起止时间、课程、级别、老师、教室、正式预约 / 候补状态和可取消截止时间。源文案为“课前 N 小时可取消”时，必须以开课时间减去 N 小时计算绝对截止时间；无法可靠解析时保留并显示源文案，不能拼出不完整的“可取消至 前可取消”。
- 训练概览：与课程卡、成长等级、课程等级组成页面顶部概览；`1501px` 以上训练概览与课程卡各占内容区三分之一，右侧第三格上下放置成长等级与课程等级两张独立面板，并与前两卡同顶、同底；`1101px–1500px` 训练概览与课程卡各占一半，两个等级面板在下一行上下排列，`1100px` 以下单列。训练概览使用 2 × 3 指标展示本周训练次数、本周训练时长、本周最喜欢的课、总训练次数、总训练时长和全部时间最喜欢的课；`560px` 以下六项单列。
- 课程计划：宽桌面左侧“课程预约”占三分之一并纵向贯穿模块；右侧占三分之二，顶部“抢课助手 / 本地抢课结果”严格各占一半，下方承载“周安排”，避免周安排为了追平预约列表被拉出大面积空白。顶部两区外框必须同顶、同底、同宽、同高；两边各自四张统计卡共用同一套四等分列宽、行高、内边距和标题 / 数值 / 说明三行居中网格，八张卡在同一桌面行内宽高一致，并压缩下方无意义留白。`1320px` 以下先显示预约，再等宽并排助手 / 结果，最后显示周安排；`860px` 以下四区按同一顺序全部单列，两组统计卡继续采用相同的两列响应式布局且不得造成整页横向溢出。预约存在明确取消截止时显示绝对时间；当天课程已经退出可取消预约快照、只剩已预约课表状态时，显示“已过可取消时段”或“课程已结束”，不得伪造具体截止时间。
- 课程预约：摘要只展示从浏览器当天开始、最近三个“实际有预约 / 候补的日期”中的全部课程，而不是简单截取前三节；当天按完整自然日纳入，不能因某节课已经开始就从摘要中删除。同一天连续多节必须完整保留，更远日期不进入摘要。摘要以日期为外层分组：同一天只渲染一个大卡，左侧统一显示 `M.D` 与真实星期，并在日期轨道内水平、垂直居中；右侧按时间顺序纵向放置当天每节课程的独立小卡。窄屏把日期移到顶部后，日期与星期继续整体居中，课程小卡仍依次排列。摘要以当前预约 / 候补快照为主，并用脱敏课表中明确为 Owner 已预约 / 排队的课程补齐当天已开始但不再位于 upcoming 的记录；两源按日期、起止时间和标准课型语义去重。“已预约”使用粉玫瑰色，“候补”使用橙色并在存在序号时显示“排队第 N 位”。每张课程小卡保留时间、课程、老师、教室、取消截止和业务状态。
- 五周周安排：周期固定为周一至周日，用户可在两周前 / 上周 / 本周 / 下周 / 两周后五档之间切换，默认本周，左右箭头到达两端后禁用。过去两周和本周优先从实际完成记录、当前预约 / 候补快照以及脱敏课表中的 Owner 状态合并去重；未来两周同样先展示已有预约 / 候补 / 完成事实。所选未来周没有任何真实课程、存在日期落在该周的公开 Fast Path `targets` 且尚无同周执行结果时，统一显示这些有明确日期的“准备抢”目标，不再把逻辑限制在固定的“下周”偏移；不得脱离公开目标自行伪造未知课程。相同日期、起止时间和标准课型的“芭蕾 L1 / 芭蕾L1-入门”或“软开 / 软开课”必须视为同一课程，预约快照和课表事实优先于 Fast Path 的泛化结果，不能把 9 节重复显示成 18 节。实际课程使用与下方完整课程表一致的左侧时间轴、长空档压缩和分钟定位；整周连续至少 60 分钟没有课程时，压缩必须从上一节实际结束分钟延续到下一节实际开始分钟，不能只按完整整点小时判断。每个日期固定拆成“大教室 / 小教室”两个子列，课程只进入对应教室列，同一教室内的时间冲突才继续分轨；当某个日期的某间教室整天无课时，该教室列自适应收窄，把空间留给有课教室，日期和教室表头仍须完整可读。教室已由列头表达，课程块不再重复教室名，只保留课程名、老师、完整起止时间与状态，并通过更紧凑的字号和间距适配半宽子列。周安排统一隐藏报名人数、容量和全班候补人数，只保留个人预约 / 候补状态；下方完整课程表继续展示人数统计。压缩空档左侧只显示空档内首个完整整点（例如真实空档从 `12:30` 开始时仍显示 `13:00`），不额外显示范围文案，也不在压缩带下沿重复显示结束刻度。时间轴底部收在全周最后一节课的准确结束时间，例如最后一节 `21:15` 结束时不再继续留白到 `22:00`。课程块继续复用课型色和预约 / 候补深浅。准备抢卡继续使用公开目标的课程、日期与规则时间，并用小字号标出真实执行优先级；任一真实课程存在时整周切回真实时间轴，不与准备抢卡混排。有同周 `lastRun.records` 后，以逐课真实课程、时间与 booked / waitlist / failure 状态替换计划；不得用滚动后的新目标回填旧结果。周安排不展示源站全部可约课程，不展示 Session、源 ID、内部路径或原始响应。桌面七天直接平铺，`860px` 以下允许周安排内部横向浏览，页面自身仍不得溢出。
- 周安排状态新鲜度：`lastRun.records` 只在 Fast Path 结果不早于芭蕾业务快照时作为即时兜底；一旦 `ballet.json.dataAsOf` / `sync.lastSuccessAt` 已晚于该次抢课，周安排必须完全以最新业务快照中的预约、候补与完成事实为准。取消后已从当前预约和 Owner 课表状态中消失的课程不得被旧抢课结果重新显示。
- 抢课助手：使用四张等宽摘要卡，分别显示公开状态累计 `totalBooked` 的“累计抢到”、累计 `totalWaitlisted` 的“累计候补”、历次真实执行关键路径耗时的“抢课平均耗时”和当前运行状态。私有 Fast Path 状态按执行时间保存每次真实抢课的 `criticalPathMilliseconds`，公开状态只发布平均值和样本数，不发布完整历史；旧状态尚无历史数组时先以现有 `lastRun` 的关键路径作为首个样本。
- 本地抢课结果：只显示总体统计，不展示逐课列表。总体统计固定为成功预约、候补中、未抢到和抢课耗时四项，前三项来源为 `lastRun.records` 的实际状态，耗时优先读取本次 `criticalPathMilliseconds`，旧记录缺失时回退本次所有正数 `elapsedMilliseconds` 之和；标题旁显示记录数与最近执行时间。已预约 / 已在排队按对应成功类归组，其他未完成、未开放、唯一性失败或安全停止归入未抢到；存在公开候补位次时可在候补副说明中汇总。不得展示 timer、重试轨迹、Session、源记录 ID 或内部运行路径。
...
- 课表老师显示优先级补充：本条替代上一条允许中等桌面或重叠窄卡隐藏老师的旧规则。老师姓名在所有课程卡中都必须显示，包括 `1101px–1500px` 中等桌面、60 分钟及以下的紧凑卡和同教室重叠窄卡；课程名后优先保留独立老师行，人数 / 排队必要时压缩为不换行的单行，时间 / 状态也保持单行，不能通过隐藏老师换取空间。
- 课表时间显示优先级补充：起止时间属于不可省略的核心事实，任何桌面课程卡都不得用省略号或裁切隐藏结束时间。60 分钟及以下紧凑卡在所有桌面宽度都把状态徽标放到完整时间下一行且两者都不收缩，优先压缩人数 / 排队文字；长课程仍可在空间充足时让时间与状态同排。
- 软开课色相补充：原“灰米色”统一解释并实现为偏奶咖的暖米色，不再使用中性或偏冷灰；普通、排队、预约和已上完仍只改变这一暖色色相的明度与深度。
- 课程表分组层级：桌面日期边界与同日教室边界都使用自然的 `1px` 细线，不画贯穿全高的粗线；日期线使用稍深的暖灰粉，教室线使用接近背景的浅灰粉，通过色阶而非粗细形成层级。移动端继续用独立日期卡片分组。完整课程表恢复原有紧凑时间刻度，60 分钟及以下课程统一复用紧凑卡规则，仅将“报名 / 容量 / 全班排队人数”行和完整起止时间缩至 `5px`，并使用 `2px` 上下内边距与紧凑行高保证底部状态完整露出；课程名、老师和可约、可排队、已满、已预约等状态标签保持与同轨 90 分钟课程一致的字号和标签尺寸。所有状态都必须位于完整时间下方，在卡片最底部左对齐独占一行，不能按状态类型局部覆盖。90 分钟等长课仍按真实时长明显拉开，长空档继续单独压缩。
- 训练记录：作为芭蕾页最后一个模块，上课统计与上课历史合并为一张全宽面板，提供“本月 / 今年 / 全部”和“节数 / 时间”切换；主值、课程类型、课程级别、授课老师、图表与历史必须同步采用同一时间范围与指标。本月使用按日历日期排布的热力图，有已加载的训练历史后即可展示；热力图外框只按七列日历所需宽度紧凑展开并左对齐，不得为了占满全宽留下大块横向空白，未来尚未纳入同步的日期使用低对比浅底虚线格，不铺密集斜纹或重复破折号。今年使用从 1 月到浏览器当前月的按月折线，不显示未来月份；全部使用仅包含实际有记录年份的年度折线。两档折线图也必须按实际时间点数量收敛并左对齐，桌面最大宽度与热力图一致为 `840px`，数据少时进一步缩短，手机端才占满可用宽度。只要所选范围存在上课记录就展示对应图表；没有记录时显示明确空状态。`1501px` 以上图表列只占图表实际所需宽度，紧凑历史紧接在其右侧并吃满剩余空间，不得在两者之间留下空列；图表标题已经包含“节数 / 小时”时不再另放单位徽标。右侧通过脱离网格行高计算的铺满层始终与左侧图表同高；历史保持两列并渲染当前范围的全部记录，列表在卡片内部纵向滑动，不再截断为最近 8 节。当前范围超过首屏容量时，标题区继续显示紧凑的“全部记录”入口，用于打开包含完整标签的详情浮层。完整历史以当前页面上的居中悬浮层打开，高度受控且课程列表只在浮层内部纵向滚动，不使用整页高的右侧抽屉，也不在卡片底部放整宽按钮。`1500px` 以下改为图表在上、历史在下的自然布局并限制历史列表最大高度，`560px` 以下历史改为单列，仍可在卡片内部上下滑动；手机浮层保留页面边距并继续内部滚动。课程级别有明确等级时显示 L1 / L1.5 等等级，没有等级时不显示“无级别”，改用软开、肌肉素质、技术技巧等真实课程类型。授课老师作为与课程类型、课程级别并列的独立分布卡，按所选节数或时间显示各老师的课程占比。只将闻道明确表示已上课 / 已完成的记录计入统计，历史保留日期、课程、展示级别 / 课型、老师、起止时间和时长。时长的权威值始终保存为整数分钟，小时只在展示层换算。
- 训练趋势与历史卡片：左侧热力图 / 折线图和右侧上课历史必须使用同族完整卡片，英文眉题、中文标题都放在卡片边框内；两张卡在桌面同顶同底。图表本身不得再保留第二层外框，避免标题悬在卡外或形成卡片套卡片。
- 课程卡：放在页面顶部训练概览右侧并保持等宽；`1501px` 以上各占内容区三分之一，`1101px–1500px` 各占一半，`1100px` 以下随概览退回单列。页面不再给课程卡套普通白色 `panel` 外框，“Course Card / 课程卡 / 使用中或已失效状态”直接进入暖象牙票券顶部；多张卡使用单卡宽度的横向吸附轨道展示，使用中卡稳定排在已失效卡之前，触屏可左右滑动，并提供上一张 / 下一张、圆点页码和键盘方向键，不把多张卡纵向撑高概览。票券只保留淡香槟金外描边和完整圆角轮廓，不使用侧边缺口、顶部内分隔线或右侧票根虚线。插画固定使用本地 `dash/assets/ballet/membership-ballerina.webp`，只作为装饰，不承载任何课程卡文字或数字；票券底色必须匹配插画自带的暖象牙纸色，不得用混合模式形成矩形色差。插画必须使用完整缩放而非铺满裁切，从抬起的手尖、横向伸出的脚尖到支撑腿足尖都可辨认；宽卡使用左侧完整舞者、右侧信息的物理分栏，右侧事实区必须从横向脚尖之后开始，不能用事实内容覆盖脚尖，也不能靠遮罩把被覆盖部分隐去；窄卡才允许作为左侧淡背景，但不得只剩局部躯干或腿部影子。卡内按卡信息、课程使用 / 有效进度、计划结论纵向收纳：使用中的完整卡显示课次水平进度和有效天数环形进度；已失效卡也保留展示卡名、有效期、失效状态和闻道最后提供的剩余课次，闻道不再提供总次数时保持为空，不猜测已用次数、不绘制虚假课次进度，也不阻断其他卡与整次同步。有效进度卡第一行只放标签和右上圆环，第二行的“第 N / 总天数”以及第三行的到期节奏说明分别独占整行，不能把完整文字组与圆环硬塞在同一横行。圆环必须比正文主比例更有视觉面积，环内当前天数与总天数固定使用单行横排 `N /总天数` 并整体居中，不能上下堆叠；正文主数字不得沿用课程使用的最大字号。计划结论只保留建议周课次、预计用完日期和预计提前天数，避免重复展开一节 / 周情景。卡片宽度达到 `650px` 时两项指标并排，低于 `650px` 时在舞者右侧或下方改为单列，标题、有效期和全部事实不得横向溢出。每张卡独立使用自己的有效期和已用课次，不能用全局上课记录推断某张卡的消耗；计划与实际节奏只对字段完整的使用中卡计算。开卡未满 28 天时不展示实际节奏、能否用完或增量结论；满 28 天后才按该卡开卡以来的 `已用课次 / 实际开卡天数` 补充实际节奏预测。不得展示会员卡号、会员 ID 或源记录 ID。
...
- 训练概览的六项统计都以 `dataAsOf`（缺失时回退最近成功同步时间）为唯一截止点，只计状态明确为已上完 / 已完成、且课程结束时间不晚于截止点的记录；未来已预约、候补、尚未开始和尚未结束的课程全部排除。训练次数按课次计数，训练时长只累加具有有效分钟数的已完成记录。“最喜欢的课”按同范围内的原始课程名统计上课次数，次数最多者胜出；并列时优先最近上完的课程，再按课程名稳定排序。缺少可信刷新时间或结束时间时采用 fail closed，不把对应记录计入概览。
...
- Owner 已确认：实际上课历史中闻道未填写老师的记录默认按“李俊”处理，并进入李俊的老师分布；2026-07-30 18:45–19:45 的软开课明确校正为“王嘉豪”，2026-08-07 19:45–21:15 的芭蕾 L1 明确校正为“张瀚泽”。2026-08-18 19:45–21:15 大教室的芭蕾 L1 因已转出且没有实际上课，虽然闻道仍返回“已上课”，也必须从公开历史与全部训练统计中排除。闻道明确填写的其他老师和 Owner 手工补录的老师保持原值。默认值、明确校正与单课排除只用于已上课历史，不套用到未来课表、当前预约或候补。
- 学习图表：同一面板提供“本月 / 今年 / 全部”周期和“节数 / 小时”指标切换，不使用双 Y 轴。本月按周一至周日的日历网格展示每日热力，尚未纳入最近成功同步覆盖的日期必须与真实 0 值区分；今年按月折线并补齐 1 月至当前月，未来月份既不显示也不记为 0；全部按周一至周日聚合成年度分组热力图，只展示实际存在训练记录的年份，并从该年首个训练周补到最后训练周或最近成功同步周。标题、周日期、热力色阶、图例和单位必须随范围与指标同步更新；同步失败时保留最后成功数据，不能把失败后的未知周画成 0。
...
- 逻辑日仍以 `Asia/Shanghai` 的 00:00 为界；生产 rolling 任务每天 `09:00 / 12:00 / 15:00 / 18:00 / 22:00` 运行，并在周日 14:30 额外读取抢课后发布的下周课表；独立本地调度器每 5 分钟只检查脱敏 read model，到达本周期最后一节结束后 10 分钟时才启动一次既有 rolling 完整刷新。任务使用单实例锁、有限网络重试；身份失效不重试。
...
- `dash/data/ballet-booking-fast.json` / `.js` 是自动抢课状态的安全 fallback；生产同 schema 文件由 root fast-path service 写入 `/var/lib/maxnow-ballet-booking-fast-public`，nginx 通过已有登录校验的 exact alias 且 `no-store` 提供。它只保存启用状态、是否允许目标课自动候补、固定优先级、脱敏目标、上次 / 下次执行、累计预约 / 候补次数、耗时样本数、关键路径平均耗时和逐课安全结果；完整历次耗时只保存在私有 Fast Path 状态，不得保存或公开课程 / 会员 / 卡片源 ID、凭据、响应正文、内部路径或 unit 名。
...
- 采集器只允许已确认的只读 GET 页面，包括严格日期格式的课程表路径，以及页面自身声明的固定上课历史 / 活动预约列表分页 POST；分页 POST 仅加载列表，不属于业务 mutation。禁止调用预约、取消、候补、转课、登录写接口或其他未知方法。rolling 同步为北京时间每天 `09:00 / 12:00 / 15:00 / 18:00 / 22:00`，周日 14:30 与 20:00 额外同步，月度 full 保持每月 1 日 00:47。
...
- 对话式实时查询使用独立无缓存 CLI 和临时 systemd unit：按问题选择课表、当前预约、上课记录或课程卡最小范围，直接返回脱敏 JSON，不读取或改写 `dash/data/ballet.*` 与 `/var/lib/maxnow-ballet`。当前预约必须通过共用活动预约分页读取器覆盖首页之后的预约与候补，再读取活动记录详情；临时 unit 结束后清除解密凭据目录；返回值不得包含源记录 ID、会员标识、原始 HTML、Cookie 或内部路径。
- 对话式显式预约使用独立 `run_ballet_booking.sh`：只接受日期、起止时间、课程名、老师和教室完全匹配的课程，并要求当前请求中明确确认。单课或多课先统一实时预检已有预约、余位、唯一可用课程卡和闻道规则；全部可执行后才按输入顺序逐节提交，每节最多调用一次 `do_addbook` 并立即从实时预约记录复核。身份失效、页面变化或结果不明确时停止后续课程且禁止盲目重试；不得输出课程 / 会员 / 卡片源 ID。预约解析必须在同一个源 `.classtable` 课程块内原子绑定课程事实与该块预约按钮，禁止把经过展示排序的课程数组与源顺序按钮数组按位置配对。
- 对话式显式取消使用独立 `run_ballet_cancellation.sh`，第一版一次只接受一节课程，并要求日期、起止时间、课程名、老师和教室与当前活动预约完全匹配且 Owner 当前请求明确确认。dry-run 必须从实时预约详情校验唯一活动预约、可见取消控件、官方规则检查与取消路径 contract，允许调用已确认无写入的 `check_cancelrules` POST，但 `mutationAttempts` 必须为 0；execute 在提交前重新读取同一预约和规则，每次最多调用一次 `do_cancel`，随后重新查询实时预约并以目标消失作为成功依据。mutation 后网络或响应不明时禁止重试；未能确认目标消失就返回 `unknown_result`。不得批量、定时或自动取消，不得输出预约记录 ID、Cookie、原始 HTML、响应正文或内部路径。
- 周日自动抢课使用独立 `book_ballet_fast.py` 与常驻 systemd timer。服务于北京时间周日 14:19:35 启动并只读预热，14:20:00 才开始提交；关键路径不经过 Codex、Skill、OpenClaw 或 SSH。每节课程按日期、课型 / 等级、起止时间和教室做唯一语义匹配，老师不参与匹配或同周幂等 occurrence 计算，因此同一目标临时更换老师时仍继续处理；如果除老师外的字段命中多节，仍以 `course_not_unique` 停止该目标。Fast Path 必须复用对话式预约的同块原子绑定结果，展示排序只能作用于完整绑定条目的副本或脱敏 read model，不能进入按钮选择链。周一至周六六个目标日期先以最多 3 路并发 GET 完成第一轮读取，随后在放课后第 2 / 6 / 10 秒以最多 2 路 GET 后台刷新；第一轮已发现的 L1 可在后台发现窗口结束前进入严格串行 mutation，稳定快照补入后发布课程后再继续 L1.5 与软开，避免“出现任意目标即收口”漏课。课程卡资格与闻道规则以最多 2 路并发提前预检，并设置 15 秒有效期，过期后在提交前重新检查。真实 `do_addbook` mutation 始终按优先级严格串行，每节最多同时存在一个 mutation；全部提交后的活动预约列表先通过共用分页读取器覆盖首页之后的预约 / 候补，再对目标日期详情采用最多 3 路并发只读核验。HTTPS 连接池保持最多 3 条 keep-alive 连接，减少重复建连。命中 `available` 时预约，命中 `queue_available` 且配置显式启用 `allowWaitlist` 时排队；已预约或已排队时不重复提交。网络波动、课程卡尚未开放、规则短暂未就绪或 `do_addbook` 明确返回 `NOTOPEN` 时，首次失败后最多安全重试 3 次，退避总时长为 0.56 秒；明确已满、已停止、无卡或不可约时不做无效重试。每节结果彼此独立，单节失败或 mutation 结果未知不得阻止后续课程；未知 mutation 不得重复 POST，统一核验未命中时仅在提交后第 0.8 / 2 / 4 秒继续只读复核，并按 `booked` / `waitlist` 区分最终结果和候补位次。只有身份失效、配置错误或页面结构变化这类全局安全问题才停止后续课程。
- 自动抢课按课程优先级 `芭蕾 L1 > 芭蕾 L1.5 > 软开 / 软开课` 排序。每一课程层内依次为周六符合时段的课程、周一至周五李俊课程、周一至周五其他老师课程；老师字段为空按李俊处理，各组内部再按周一至周五与开始时间保持确定性。周六不按老师分层；工作日同一日期、课程名与时间优先选择李俊老师层，未命中才选择其他老师层，每层教室仍按大教室优先、小教室兜底。周一至周五只处理 18:40（含）后开始的目标；周六按结束时间判断，只处理结束时间严格早于 18:00 的目标，18:00 整结束也排除；周日排除。周安排的准备抢卡必须原样展示“18:40 后 / 18:00 前结束”规则边界，旧数据里的周六“全天”也必须按现行规则显示为“18:00 前结束”。可预约则预约，可排队则候补；配置外课程一律跳过，不取消、转课、买卡、支付或登录。
...
- 新规则默认先以 `off` / `dry-run` 验证；Owner 也可以对明确列出的课程、顺序和时间单独批准直接进入 `enabled`，但必须在规格、配置和更新记录中留下该次授权边界。2026-08-09 Owner 明确将已启用目标替换为周日 17:30–19:00 芭蕾 L1 与 19:00–20:00 肌肉素质两节大教室课程，并明确只抢这两节；此前七项目标授权不再生效。
- 真实提交前必须即时校验课程唯一性、已有预约、余位、会员卡和闻道规则；私有幂等账本防止同一目标重复提交。关键路径不做整批前置详情核验，也不重试 mutation，避免把延迟或未知响应扩大为重复预约。
- 默认不自动取消、转课、买卡、支付或报名付费活动；专用取消 runner 只用于 Owner 当前对话中明确指定并确认的单节预约。遇到验证码、微信重新授权、页面结构变化或无法判断的响应时立即停止。
