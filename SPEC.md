...
- 课程卡：使用独立全宽面板展示卡名、剩余 / 总课次、已用课次、有效期和同步时间，不内嵌进预约或上课统计。按近 28 天实际上课节数除以 4 计算当前周频率，并以剩余课次 / 剩余有效周数计算到期前所需周频率；展示按当前频率能否用完、建议每周至少上几节和相对当前频率还需增加多少。不得展示会员卡号、会员 ID 或源记录 ID。
