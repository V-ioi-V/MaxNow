...
- 课程计划：宽桌面左侧“课程预约”占三分之一并纵向贯穿模块；右侧占三分之二，顶部并排“抢课助手 / 上次抢课结果”，下方承载“周安排”，避免周安排为了追平预约列表被拉出大面积空白。`1320px` 以下先显示预约，再并排助手 / 结果，最后显示周安排；`860px` 以下四区按同一顺序全部单列且不得造成整页横向溢出。预约存在明确取消截止时显示绝对时间；当天课程已经退出可取消预约快照、只剩已预约课表状态时，显示“已过可取消时段”或“课程已结束”，不得伪造具体截止时间。
...
- 上次抢课结果：只显示总体统计，不展示逐课列表。总体统计固定为成功预约、候补中、未抢到三项，来源为 `lastRun.records` 的实际状态；标题旁显示记录数与最近执行时间。已预约 / 已在排队按对应成功类归组，其他未完成、未开放、唯一性失败或安全停止归入未抢到；存在公开候补位次时可在候补副说明中汇总。不得展示 timer、重试轨迹、Session、源记录 ID 或内部运行路径。
