...
- 未来课程：芭蕾页最顶部只保留低权重更新时间和一张全宽紧凑“下一节预约”卡；预约卡按“紧凑日期 / 弹性课程信息 / 状态”三段自适应分配宽度，标题与课程信息共用左基线，不使用固定宽度内容块强行居中。独立“所有预约”列表同时完整展示所有尚未上课的已预约 / 排队中课程，包括顶部的下一节，不显示已取消或已经结束的课程。列表的日期列上下展示日期与真实星期，课程列只保留时间和老师；“已预约”使用粉玫瑰色，“候补”使用橙色并在存在序号时显示“排队第 N 位”，不能只写笼统的“排队中”。后续按周扩展可预约、满员和已取消状态；剩余名额必须标记为抓取时快照，不能当作持续有效承诺。
