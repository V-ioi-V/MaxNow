...
- PHPSESSID 活跃实验：作为页面最底部默认收起的原生折叠详情，主值为截至最后一次 `authenticated` 样本的“已确认有效时长”，并展示原始实验起始、最近自动检查、下一次自动检查和当前检查间隔；身份异常只在页面顶部保留短告警。该详情只能写“自动检查 / 只读探针”，不能称为“自动续期”；失败或停止后必须冻结在最后成功证据，不能按浏览器当前时间继续外推。服务器每 5 分钟发布一次本地脱敏状态；即使 JSON / localStorage 仍可读，只要 `updatedAt` 超过 15 分钟未更新，前端也必须从绿色运行态降级为检查延迟。
...
- 本周训练：使用独立面板展示本周已完成、已预约、候补、预计节数和对应时长；预计采用“正式预约”为下限、“正式预约 + 候补”为上限，不能把候补直接算成一定会完成。宽桌面与所有预约、课程卡组成最顶部三列，窄屏排在所有预约之后、课程卡之前。
- 课程卡：使用独立面板展示卡名、开卡日期、开卡第几天、剩余 / 总课次、已用课次、有效期和同步时间，不内嵌进本周训练、预约或上课统计。宽桌面与所有预约、本周训练组成最顶部三列，窄屏自然堆叠。每张卡独立使用自己的有效期和已用课次，不能用全局上课记录推断某张卡的消耗。每天以剩余课次 / 剩余有效周数计算到期前所需周频率，并展示“按建议整数周课次预计何时用完”和“若每周 1 节到期约剩多少”的计划情景。开卡未满 28 天时不展示实际节奏、能否用完或增量结论；满 28 天后才按该卡开卡以来的 `已用课次 / 实际开卡天数` 补充实际节奏预测。不得展示会员卡号、会员 ID 或源记录 ID。
- 本周课程表：三个核心卡下方展示源站当天全部课程，不只展示 Owner 的预约。桌面为带内部横向滚动的时间 × 日期网格，`860px` 以下按日期分组。当天使用粉玫瑰描边高亮，已经过去的日期降低饱和度和不透明度，后续日期保持普通色；已预约课程使用粉玫瑰整卡高亮，排队中课程使用橙色整卡高亮，二者必须清楚区分。通常展示当周周一至周日；周日 14:20 额外同步，只有确认下周至少存在一节课后才切换为“本周日 + 下周一至周日”共 8 天，并隐藏本周一至周六；未拿到下周数据时继续保留本周。
...
- 未来课程：芭蕾页在低权重更新时间下方使用“所有预约 / 本周训练 / 课程卡”三个同级面板；宽桌面同排同高，`1200px` 及以下按该顺序单列堆叠。“所有预约”完整展示所有尚未上课的已预约 / 排队中课程；第一条即下一节，使用大号粉白主卡并标记“下一节”，保留日期、课程、状态、老师和取消截止，其余预约使用紧凑小行形成主次。日期列上下展示日期与真实星期，“已预约”使用粉玫瑰色，“候补”使用橙色并在存在序号时显示“排队第 N 位”，不能只写笼统的“排队中”。`560px` 以下列表行自然堆叠且不得横向溢出。列表不显示已取消或已经结束的课程。后续按周扩展可预约、满员和已取消状态；剩余名额必须标记为抓取时快照，不能当作持续有效承诺。
