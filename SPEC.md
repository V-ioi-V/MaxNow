...
- 自动抢课按课程优先级 `芭蕾 L1 > 芭蕾 L1.5 > 软开 / 软开课` 排序，再在每一课程层内按日期优先级 `周六 > 周一 > 周二 > 周三 > 周四 > 周五` 排序，同课程、同日期内按开始时间保持确定性。周一至周五只处理 18:40（含）后开始的目标，周六处理全天，周日排除；老师不限，教室按大教室优先、小教室兜底。可预约则预约，可排队则候补；配置外课程一律跳过，不取消、转课、买卡、支付或登录。
