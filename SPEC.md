...
- 芭蕾页顶部在“已同步”状态右侧提供小红书“芭蕾周记录”入口；`week N` 继承同一状态胶囊的字号、字重和高度。弹窗只展示当前周封面，固定为 `1280×1710`，支持预览、下载和在浏览器能力允许时复制到剪贴板。
...
- 进入芭蕾页后利用浏览器空闲时间预生成默认 `week N` 封面；同周、同模板版本时复用当前页面缓存。底图、手绘数字位置与版本通过 `dash/assets/ballet-week-cover/template.json` 管理；生成只发生在浏览器，不上传或在服务器保存 PNG 成品。周简报及其周期收尾刷新任务已于 2026-09-20 停用。
