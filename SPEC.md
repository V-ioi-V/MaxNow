...
- 当前 v1 仍保留六个一级入口，不在规划阶段提前修改导航；只读 MVP 数据稳定后，再评估将“芭蕾 / 学习与约课”作为独立 `secondary-view` 页面。
...
- 下一节课：日期、周几、起止时间、课程、级别、老师、正式预约 / 候补状态、可取消截止时间。
- 本周训练：目标节数、已约节数、已完成节数和预计总时长。
- 未来课程：按周查看已预约、候补中、可预约、满员和已取消状态；剩余名额必须标记为抓取时快照，不能当作持续有效承诺。
- 上课记录：与预约记录分开，按日期、课程、级别和老师查看实际完成记录。
- 学习进度：累计节数与时长、近 30 天频率、课程类型 / 级别 / 老师分布。
...
- 实施时可新增 `dash/data/ballet.json` / `dash/data/ballet.js` 作为脱敏 read model；规划阶段不提前创建空数据文件，也不把它写成已存在的数据契约。
- 未来 read model 至少区分 `meta`、`summary`、`upcoming`、`attendance`、`learningLogs`、`authHealth` 和 `automation`；课程与记录必须使用稳定键去重，预约状态和上课状态不得混为一类。
- 所有模块都要展示来源时间、新鲜度和失败状态。Session 失效或同步失败时保留最后成功快照并标记 `auth_required`、`stale` 或 `error`，不得伪装成“暂无课程”。
