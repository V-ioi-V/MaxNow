...
- `scripts/book_ballet_fast.py` / `scripts/run_ballet_booking_fast.sh`：周日 14:20 自动抢课入口；读取版本化长期规则，第一轮日期页读取后只提前处理已发现的周六 L1，同时在放课后第 2 / 6 / 10 秒以最多 2 路 GET 后台刷新完整日期集合，等待发现窗口结束后，按稳定快照依次补入周六剩余课程，再处理周二、周五、周一、周三、周四；各天内部保持 L1 / L1.5 / 软开顺序。真实 mutation 始终严格串行；结果未知时不重复 POST，统一核验与第 0.8 / 2 / 4 秒追加复核都通过共用活动预约分页读取器确认首页之后的候补。生产 systemd 单元在每次执行结束后，无论主流程成功或失败，都立即触发一次独立的业务只读 rolling 同步，刷新 MaxNow 芭蕾面板；同步只允许 GET 与上述两个固定列表分页 POST，结果不覆盖抢课单元自身结果。
...
- 当前 Fast Path 每周扫描放课后的周一至周六课表：周一至周五只处理 18:40（含）后开始的课；周六按课程结束时间判断，只处理结束时间严格早于 18:00 的课，18:00 整结束也排除。动态处理所有标准芭蕾 L1、标准芭蕾 L1.5 与课程名严格等于“软开”或“软开课”的课程，周日不进入目标。软开专项、软开-胯及其他只因分类器含“软开”而归入同类的近似课程必须排除。先按日期优先级周六 → 周二 → 周五 → 周一 → 周三 → 周四，再在每天内部按 L1 → L1.5 → 软开 / 软开课排序；工作日同一天同课型先李俊（老师空白按李俊）、再其他老师，各组内按开始时间排序。周六不按老师分层；工作日同一日期、课程名与时间先选择李俊老师层，未命中才选择其他老师层，每个老师层内仍按大教室优先、小教室兜底并要求唯一匹配；可预约则预约、仅可排队且 `allowWaitlist=true` 时才候补。
...
- 2026-09-20：Owner 停用周简报；删除浏览器简报生成与专用周期收尾调度器、systemd service / timer，以及同步器的 weeklyBrief 派生字段。日常五次 rolling、周日 14:30、每月 full 和抢课结束后的同步保持原有边界，所有预约类入口与手工上课记录不变。

- 2026-09-20：Owner 将抢课改为日期优先：周六 → 周二 → 周五 → 周一 → 周三 → 周四，每天内部 L1 → L1.5 → 软开。只允许周六 L1 在首轮发现后提前提交，其余目标等待发现窗口结束后按新顺序处理，避免旧整周 L1 流水线越过高优先级日期；schema v10，时间门槛、老师 / 教室选择、串行 mutation 与幂等边界不变。

- 2026-09-14：Owner 澄清周六“18:00 前”指课程结束时间严格早于 18:00；Fast Path 从按开始时间筛选改为按结束时间筛选，18:00 整结束也排除。工作日 18:40（含）后开始、课程 / 老师 / 教室优先级、候补、幂等与失败关闭边界不变。
...
- 2026-09-13：Owner 将周六时段收窄为仅处理 18:00 前开始的课程；18:00 整及之后的周六课程必须排除，工作日 18:40（含）后的门槛不变。

- 2026-09-11：适配闻道会员卡页同时返回使用中卡与已失效旧卡；已失效卡只有“卡内余”而无“总次数”时继续脱敏展示，使用中卡的完整次数校验与全部预约 mutation 边界不变。

- 2026-08-23：Fast Path 保持课程层 `L1 → L1.5 → 软开` 不变，在每层加入老师优先级：周六全部课程优先，其后先处理周一至周五李俊课程（老师字段为空按李俊），再处理周一至周五其他老师；教室、候补、幂等及失败关闭边界不变。
