...
- 当前 Fast Path 每周扫描放课后的周一至周六全天课表，动态处理所有标准芭蕾 L1 与课程名严格等于“软开”的课程；周日课程不进入目标。软开专项、软开-胯及其他只因分类器含“软开”而归入同类的近似课程必须排除。目标不限老师；同一日期、课程名与时间先在大教室中唯一匹配，大教室没有时再在小教室中唯一匹配；可预约则预约、仅可排队且 `allowWaitlist=true` 时才候补。
...
- 2026-08-16：Owner 将 Fast Path 改为长期周规则：每周日放课后扫描周一至周六全天，预约所有标准芭蕾 L1 与精确“软开”；严格排除软开专项、软开-胯，后续不再要求 Owner 每周提供固定课表。
