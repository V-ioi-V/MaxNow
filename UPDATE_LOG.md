...
- 当前固定 20 分钟的 Session 活跃寿命 / 滑动窗口实验继续运行。为避免额外 GET 污染实验频率，生产每日 00:17 抓取保持待启用；该实验只能证明持续活动寿命，不能证明空闲 Session 寿命。
...
- 主分支 `7a57225` 已部署到服务器；四个生产 unit 已安装并通过 Linux 测试、项目检查和 nginx 校验，但两个 timer 保持 disabled、enable gate 不存在。部署前运行数据已备份并完整恢复，当前 20 分钟实验继续 active。
