...
- Home 顶部右侧小日历增加北京市海淀区天气摘要，显示地点、天气、当前温度和今日高低温。
- 新增 `dash/data/dashboard.json.weather` 只读字段，并重新生成 `dash/data/dashboard.js`。
- 前端只读取本地数据展示，不在浏览器端请求外部天气接口。
- 将 Dash 缓存版本提升到 `styles.css?v=49`、`app.js?v=40`。

原因：

- Owner 希望在时间卡片区域看到今天的天气、温度，地址为北京市海淀区。
