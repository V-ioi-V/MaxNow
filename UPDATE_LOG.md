...
- 顶部三卡保持等宽等高，但将本周训练和课程卡从平均四宫格改为“一主三辅”：突出已预约与剩余课次，其余指标收为紧凑辅助行；所有预约继续用大号下一节与紧凑后续课程形成对比。
- 上课统计把总数、课程类型和级别合并为带分隔线的统一信息带；趋势图降到 `224px`，只标真实上课日，零值不再逐日显示，并用低饱和面积提示有效区间。样式缓存提升到 `styles.css?v=155`，脚本缓存提升到 `app.js?v=128`，版本提升到 `1.0.5.24`。
