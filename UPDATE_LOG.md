...
- 保存 Owner 提供的李俊芭蕾《课程类型与选课指南》完整原图，并在芭蕾顶部状态区新增低权重“选课指南”入口，点击后可放大查看或另存。
- 原图放在受登录保护的 `private-assets` 路径并按需打开，不进入公开素材区或芭蕾运行态数据，也不改变课程等级或抢课优先级；样式缓存提升到 `styles.css?v=273`，版本提升到 `1.0.11.25`。
