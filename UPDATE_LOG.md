...
- 服务器已通过已有账号密码和一次人工确认的图形验证码刷新登录态；`gen_checkin_data.py` 现在校验 HTTP 状态、登录页标题和非空日用量，失败时保留最后有效历史并标记 `stale / last_error`，不再推进 `updatedAt`。
- 已补抓 9 月 4–7 日真实流量，线上历史恢复为 60 条，账号与流量快照更新到 9 月 8 日；豆奶凭据文件收紧为 `root:root 0600`，两个运行脚本收紧为 `root:root 0700`。
