...
- 只读检查线上 `dash/data/dounai_checkin.json`、root 豆奶签到脚本、`gen_checkin_data.py` 和当天签到日志。
- 确认当前只有签到奖励记录、账号剩余流量快照和日均可用流量；`account_history` 从 2026-06-21 才开始积累，截至 2026-07-05 只有 15 天，不足完整 30 天。
- 未发现现有脚本已经读取账单、明细、历史使用量或每日消耗接口；当前生成脚本只访问豆奶用户面板并抓取剩余流量与有效期。
- 相邻账号余量快照可做消耗估算，但不是账单级真实数据；2026-06-21 到 2026-07-05 净减少约 30.0GB，同期签到奖励约 6.7GB，估算窗口消耗约 37.4GB。
- 后续若要上线“近 30 天用量”，需要等待余量快照满 30 天，或先确认豆奶是否有可读的真实用量 / 明细接口；前端仍保持只读，不增加账号写入或手动录入。
