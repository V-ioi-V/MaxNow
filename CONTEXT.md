...
- 2026-08-02 Owner 将无人值守自动抢课的老师条件改为“不限老师”：生产 fast path 仍于周日 14:19:35 启动、14:20:00 提交，固定跨日期优先级仍为周六 > 周日 > 周五 > 其他日期；当前实际顺序是周五软开、周五 L1、周二软开、周二 L1、周四软开，均为原晚间时段和大教室。匹配与 occurrence 幂等键只使用日期、课型 / 等级、起止时间和教室；老师临时变化不会阻止预约或候补，其他字段仍必须唯一命中。每节独立执行，可预约则预约、可排队则候补；可确认安全的临时失败最多重试 3 次。单节失败或 mutation 结果未知继续后续课程，未知 mutation 本身不重试，最后统一核验预约 / 候补与候补位次。身份失效、配置错误或页面结构变化仍全局停止；不取消、转课或支付。
