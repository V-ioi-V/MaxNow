...
- `dash/data/ballet.json`：芭蕾页面与 Home 摘要读取的脱敏 read model，保存同步状态、累计 / 月度 / 年度聚合、实际上课记录、预约 / 候补与取消截止、本周日期范围、课程卡预测、周课表，以及由已预约 / 实际完成课程推导的周期最后一节、收尾刷新和周简报生成时间。顶部训练概览由浏览器从脱敏实际上课记录计算，并以 `dataAsOf` 为截止点排除未来预约、候补和尚未结束的课；Owner 确认已经转出且未实际上课的 2026-08-18 19:45–21:15 大教室芭蕾 L1 也按单课精确键从公开历史与所有统计中排除，私有账本继续保留闻道源事实。训练周简报复用同一计算函数，以每周期最后一节的实际结束时间为截止点，结束后 10 分钟刷新数据、20 分钟生成。课表通常为本周 7 天；周日 14:30 只读同步拿到下周课程后变为本周日 + 下周 7 天。周安排以小时为标尺并按实际起止分钟定位课程；整周至少连续 60 分钟无课时按真实分钟边界压缩，空档内只保留首个完整整点标签，时间轴末端停在最后一节真实结束分钟。每天固定分为大教室 / 小教室两列，同教室重叠课程只在对应教室列内继续分轨；当天无课的单间教室列自适应收窄，把宽度留给有课教室。宽桌面七天直接平铺，窄屏只在周安排内部横向浏览，页面自身不溢出。不保存 Cookie、会员卡号、会员标识、源记录 ID 或原始响应。
...
- 服务器私有 canonical ledger 是上课事实唯一机器真相：首次全量回填，日常重扫最近 60 个逻辑日并幂等 upsert，每月 1 日 00:47 全量校验；rolling 任务每天 `09:00 / 12:00 / 15:00 / 18:00 / 22:00` 运行，并在周日 14:30 额外检查抢课后发布的下周课表。`maxnow-ballet-week-closeout.timer` 每 5 分钟只检查脱敏 read model，到本周期最后一节结束后 10 分钟时启动一次既有 rolling；浏览器在结束后 20 分钟且数据刷新完成后生成周简报。同步失败保留上次成功账本与聚合，只更新独立 `sync-state.json`；预约、课程卡与课表分别保存脱敏快照。
