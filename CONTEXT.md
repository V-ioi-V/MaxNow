...
- 生产凭据已于 2026-07-27 使用新会话重新密封为 host-bound systemd 加密凭据，通过 `LoadCredentialEncrypted` 注入；页面、同步器和 unit 已部署，生产 enable gate 与 timer 仍未启用。身份失效时采集器保留旧缓存、记录脱敏错误并停止重试，直到检测到非敏感凭据版本变化；Owner 看到的操作提示固定为在电脑微信重新登录后安全刷新凭据，不展示任何 Session 值。
...
- 2026-07-27 00:26:55 启动独立 v6 新会话实验，固定每 20 分钟、最长 30 天且服务器重启后不自动恢复；首条只读课程列表样本于 00:26:56 返回 HTTP 200 / authenticated、`attempts=1`、无 `Set-Cookie`、无会话变化。v6 凭据仅以 root `0600` 的 host-bound 加密文件保存在服务器，并同步刷新了仍处于禁用状态的生产凭据版本；没有输出 Cookie 明文。
...
3. 闻道 v6 新会话生命周期实验正在按每 20 分钟运行；持续观察身份失效、Session 轮换与 `Set-Cookie`。生产 00:17 同步继续关闭，只有 Owner 单独批准后才启用并验证首次全量、增量去重、身份失败阻断和旧缓存保留，再推进 personal-wiki 学习记录同步。
