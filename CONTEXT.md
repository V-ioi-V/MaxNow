...
- 2026-08-02 Owner 确认芭蕾每周小红书封面采用固定粉色手作系列，当前周为 `week 2`，北京时间每逢周日加一。为避免 AI 重生成造成字体和版式漂移，已先建立可长期复用的透明手绘 `0–9` 数字素材与 manifest；页面 Week 入口、固定底图、本地拼图、下载和复制仍未实现，继续以 `ROADMAP.md` Later 阶段 2.6 为准。
