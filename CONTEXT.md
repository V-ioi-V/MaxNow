...
- `dash/data/token-usage.json` 是 Token 页统一入口；OpenClaw、Codex Windows / macOS、Codex server 和后续其他来源都应合入这个总账。Token 页来源列表显式展示来源，并且来源 token、费用和 runs 跟随当前选择的 `1d` / `7d` / `30d` / `all` 范围更新。
