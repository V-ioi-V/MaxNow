...
- 这些实验和生产同步都没有提交预约 / 候补 / 取消 / 转课，也不能证明精确静默闲置寿命。Owner 已批准启用每日 00:00、周日 14:20 rolling 和每月 1 日 00:47 full 只读 timer；后续约课仍按 `ROADMAP.md` 的 Later 分阶段推进。`PHPSESSID` 必须按约课网站密码处理，不得进入 Git、前端、日志、备份、聊天、环境变量或命令参数；删除本地凭据不等于吊销闻道服务端会话。
...
3. 闻道 v6 新会话生命周期实验正在按每 20 分钟运行；持续观察身份失效、Session 轮换与 `Set-Cookie`。生产每日 00:00、周日 14:20 与每月 full timer 已启用；后续观察课表发布和身份阻断，再决定是否推进 personal-wiki 学习记录同步。
