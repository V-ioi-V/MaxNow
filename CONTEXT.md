...
- 当前芭蕾页的信息顺序是：顶部概览（训练概览 + 课程卡 + 成长 / 课程等级）→ 课程计划 → 完整本周课程表 → 训练记录。课程计划宽桌面左侧“课程预约”占三分之一并贯穿模块，右侧顶部并排“抢课助手 / 上次抢课结果”、下方放占满右区的“周安排”；这样运行摘要先于课表扫读，周安排也不再被左侧长预约列表强制拉出空白。课程预约按完整自然日保留最近三个有课日期，并用课表 Owner 状态补齐当天已经开始、因而退出 upcoming 的课程；只剩当天已预约课表事实且取消字段已退出快照时，页面显示已过可取消时段 / 课程已结束，不伪造截止时间。周安排按日期、时间与标准课型语义去重，真实课程复用完整课表卡片样式，Fast Path 泛化名称不能与预约快照重复成两节。下周抢课前仍显示公开目标规则与小字号执行优先级。训练记录固定最后，并继续按本月 / 今年 / 全部与节数 / 时间统一驱动课型、级别、老师和历史。
