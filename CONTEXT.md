...
- `dash/data/ballet.json`：芭蕾页面与 Home 摘要读取的脱敏 read model，保存同步状态、累计 / 月度 / 年度聚合、实际上课记录、预约 / 候补与取消截止、本周日期范围、课程卡预测和周课表。顶部训练概览由浏览器从脱敏实际上课记录计算，并以 `dataAsOf` 为截止点排除未来预约、候补和尚未结束的课；训练周简报直接复用同一计算函数，但把截止点固定为最近一个已经到达的周日 18:00。课表通常为本周 7 天；周日 14:30 只读同步拿到下周课程后变为本周日 + 下周 7 天。周安排以小时为标尺并按实际起止分钟定位课程；整周至少连续 60 分钟无课时按真实分钟边界压缩，空档内只保留首个完整整点标签，时间轴末端停在最后一节真实结束分钟。每天固定分为大教室 / 小教室两列，同教室重叠课程只在对应教室列内继续分轨；当天无课的单间教室列自适应收窄，把宽度留给有课教室。宽桌面七天直接平铺，窄屏只在周安排内部横向浏览，页面自身不溢出。不保存 Cookie、会员卡号、会员标识、源记录 ID 或原始响应。
