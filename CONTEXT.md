...
- 固定 25 分钟阶段已于 2026-07-26 23:28 在首个标准检查返回登录失效并停止；最后认证样本为 23:03，已确认有效时长 3 小时 56 分 06 秒。三阶段结果属于可行性证据，不是最终登录方案，也不能证明精确空闲寿命或滑动续期；unit、原始日志、凭据挂载和停止方式只在 `SERVER_RUNBOOK.md` 维护。生产 00:17 抓取继续待启用，阻塞点改为刷新加密凭据和首次受控验证。
