...
- 芭蕾 Fast Path 当前使用分批放课流水线：先按日期优先级周六 → 周二 → 周五 → 周一 → 周三 → 周四，再在每天内部按 L1 → L1.5 → 软开 / 软开课排序；工作日同一天同课型先李俊（老师空白按李俊）、再其他老师，各组内按开始时间排序。14:20 第一轮课表后只提前处理已发现的周六 L1，后台在第 2 / 6 / 10 秒继续刷新六个目标日期，等发现窗口结束后按稳定快照完成周六剩余课程，再按日期优先级处理后续各天；真实预约 / 候补始终单线程，未知 mutation 不重复提交，只在第 0.8 / 2 / 4 秒追加限定日期的只读核验。配置与实现入口为 `config/ballet-booking-fast.json`、`scripts/book_ballet_fast.py`，生产操作以 `WENDA_BOOKING_AGENT.md` 和 `SERVER_RUNBOOK.md` 为准。
- 芭蕾周安排的“准备抢”卡直接展示规则边界：工作日为“18:40 后”，周六为“18:00 前结束”；为避免旧的最后成功缓存误导，历史周六“全天”在前端按现行规则归一为“18:00 前结束”。这只是展示兼容，不触发闻道操作。
...
- `dash/data/ballet.json`：芭蕾页面与 Home 摘要读取的脱敏 read model，保存同步状态、累计 / 月度 / 年度聚合、实际上课记录、预约 / 候补与取消截止、本周日期范围、课程卡预测、周课表。顶部训练概览由浏览器从脱敏实际上课记录计算，并以 `dataAsOf` 为截止点排除未来预约、候补和尚未结束的课；Owner 确认已经转出且未实际上课的 2026-08-18 19:45–21:15 大教室芭蕾 L1 也按单课精确键从公开历史与所有统计中排除，私有账本继续保留闻道源事实。课表通常为本周 7 天；周日 14:30 只读同步拿到下周课程后变为本周日 + 下周 7 天。周安排以小时为标尺并按实际起止分钟定位课程；整周至少连续 60 分钟无课时按真实分钟边界压缩，空档内只保留首个完整整点标签，时间轴末端停在最后一节真实结束分钟。每天固定分为大教室 / 小教室两列，同教室重叠课程只在对应教室列内继续分轨；当天无课的单间教室列自适应收窄，把宽度留给有课教室。宽桌面七天直接平铺，窄屏只在周安排内部横向浏览，页面自身不溢出。不保存 Cookie、会员卡号、会员标识、源记录 ID 或原始响应。
- 周安排的数据合并边界：Fast Path `lastRun.records` 仅用于业务同步尚未追上的即时状态；当 `ballet.json` 成功快照更新得更晚时，周安排只保留其中仍可由当前预约 / 候补、Owner 课表状态或完成记录确认的课程，避免取消课程被旧抢课状态复活。
...
- `dash/data/ballet-booking-fast.json`：芭蕾“抢课”工作区与 Cloud 自动抢课运维卡共用的安全 fallback；生产状态由周日 fast-path service 写入独立 public 目录，经登录校验和 `no-store` exact alias 提供。芭蕾页用它渲染累计已抢到、累计候补、历次真实执行关键路径平均耗时、本次关键路径耗时和本次总体结果；私有状态保存每次执行的时间与关键路径耗时，公开状态只发布平均值、样本数和最后一次安全结果。累计值分别读取 `totalBooked` / `totalWaitlisted`，不与 `ballet.json` 当前未来预约 / 候补列表数量混用。Cloud 读取启用状态、优先级、上次 / 下次执行和累计数；该数据不作为课程余位事实源。
...
- `scripts/sync_ballet.py`：只访问已确认的闻道业务只读页面，从服务器私有 canonical ledger 增量去重并刷新脱敏 `dash/data/ballet.*`；同时读取预约、候补、上课记录、课程卡概览和严格日期路径的周课表。上课历史首屏少于累计数时只调用官方脚本声明的固定分页 POST；摘要行必须按日期与课程名唯一命中既有私有账本，不能据此新建或猜测详情。活动预约则校验预约页声明的总数、`customerid` 与固定 `newbookrecord` contract，读取全部分页、按源记录 ID 合并去重并筛出预约 / 候补后才读取详情；该读取器同时供实时查询和 Fast Path 复核使用。同步不调用预约、取消、候补或转课写接口。
- `scripts/query_ballet_live.py` / `scripts/run_ballet_live_query.sh`：面向对话请求的实时只读入口。通过临时 systemd unit 解密服务器 host-bound PHPSESSID，按课表、当前预约、上课记录或课程卡最小范围直接读取闻道并输出脱敏 JSON；上课记录可使用同一固定分页 POST，但若查询范围包含只有摘要、没有详情 ID 的旧记录则失败关闭；当前预约使用共用分页读取器，不再受首页 10 条限制。不读取或写入 Dashboard 缓存与服务器私有账本。
...
- `scripts/cancel_ballet.py` / `scripts/run_ballet_cancellation.sh`：Owner 当前请求中明确指定活动预约后的单课取消入口。使用保存的服务器 Session 精确匹配日期、起止时间、课程名、老师和教室，校验详情页官方取消 contract，以非变更 `check_cancelrules` POST 预检；execute 重检后最多一次 `do_cancel`，再实时确认目标已从活动预约消失。未知 mutation 不重试，不输出预约记录 ID、响应正文或凭据痕迹。
- `scripts/book_ballet_fast.py` / `server/maxnow-ballet-booking-fast.*`：周日 14:20 无人值守自动抢课关键路径。14:19:35 启动并预热，放课后动态扫描周一至周六，不再依赖每周固定时间表；周一至周五只处理 18:40（含）后开始的课，周六只处理结束时间严格早于 18:00 的课，18:00 整结束也排除。所有标准芭蕾 L1、标准芭蕾 L1.5 与课程名严格等于“软开”或“软开课”的课程进入目标，软开专项、软开-胯和周日课程明确排除。先按日期优先级周六 → 周二 → 周五 → 周一 → 周三 → 周四，再在每天内部按 L1 → L1.5 → 软开 / 软开课排序；工作日同一天同课型先李俊（老师空白按李俊）、再其他老师，各组内按开始时间排序。六天课表最多 3 路并发读取，课程卡与规则最多 2 路并发预检并设 15 秒有效期，HTTPS 保持最多 3 条 keep-alive 连接，真实 mutation 始终严格串行；最终核验先用共用活动预约读取器翻完预约页，再对目标日期详情最多 3 路并发读取，因此首页之后的候补也能被确认。老师不进入 occurrence 幂等键；工作日同一日期、课程名和时间先选李俊老师层，未命中再选其他老师层，每层仍按大教室优先、小教室兜底并要求唯一命中；周六不按老师分层。目标课可预约则预约、仅可排队则候补，已预约 / 已排队不重复提交；可确认未产生 mutation 的临时失败最多重试 3 次，mutation 结果未知不重复提交但不阻止后续课程，全部结束后统一核验 booked / waitlist。每次真实执行把 `attemptedAt + criticalPathMilliseconds` 追加到私有耗时历史，公开状态只发布样本数与平均值；私有幂等状态和公开脱敏状态继续分目录保存。抢课单元每次结束后通过 systemd `OnSuccess` / `OnFailure` 立即触发独立业务只读 rolling 同步，使 `ballet.*` 面板数据跟进最新预约 / 候补；同步只允许 GET 与固定上课历史 / 活动预约列表分页 POST，失败不覆盖抢课单元结果。
- `WENDA_BOOKING_AGENT.md`：闻道抢课相关任务的专用 Agent 入口。`AGENTS.md` 只做条件路由；专用文件维护服务器 Session 优先、官方协议只读确认、最小单次 mutation、独立实时复核与交互登录降级边界。任何相关脚本、runner、配置、systemd 入口或协议边界变化必须在同一分支同步更新该文件。
...
- 2026-09-08 已修复豆奶 HTTP 401、空流量行和账号关键字段缺失被误判成功的问题：抓取失败必须保留最后有效数据、标记 `stale / last_error` 且不得推进伪成功时间；登录态可用服务器既有账号密码配合人工图形验证码刷新。豆奶签到页另有独立计算验证码，Owner 决定以后手动签到；root crontab 已移除 09:00 签到预检与微信提醒，只保留 00:05 的只读日结。该日结会从 `/user/record` 回读 Owner 手动签到结果，从 `subscription-userinfo` 的 `expire` 更新账号到期时间，并抓取昨天及更早的真实流量；不得提交签到、消耗验证次数或发送签到提醒。2026-09-08 已回读当天 20:03 的 865 MB / 1 豆丁 / 1.07 小时记录，累计恢复为 62 天，账号与流量状态均恢复正常。
...
- 周安排左侧时间轴已移除压缩空档起始时间和底部末课结束时间两个冗余标签；压缩结构、精准收口、普通整点与课程卡完整时间均保留。
- 训练记录初次打开默认展示“全部”，主值、三张分布卡、历年趋势和历史使用完整记录口径；“本月 / 今年”保留为手动筛选。
...
- 实际上课历史的老师字段按 Owner 口径归一化：闻道为空时默认补为“李俊”；2026-07-30 18:45–19:45 软开课明确校正为“王嘉豪”，2026-08-07 19:45–21:15 芭蕾 L1 明确校正为“张瀚泽”。闻道明确姓名与其他 Owner 手工补录保持原值。实时上课查询和 Dashboard 同步共用该规则，未来课表、当前预约和候补不使用默认老师或历史校正。
- 独立 `secondary-view` 页面名称“芭蕾”、副标题“课程与进度”，入口位于 Token 与云服务之间；完整顺序为首页 → 豆奶 → Token → 芭蕾 → 云服务 → 生活 → 同行记。页面使用粉玫瑰 + 白卡语义但不另做宣传型视觉；导航图标用舞鞋而非心形。
- 周安排固定提供两周前、上周、本周、下周、两周后五周窗口，默认本周并在窗口两端禁用对应箭头。过去周从脱敏上课历史读取，当前与未来周合并预约 / 候补及 Owner 课表事实；所选未来周没有真实课程、但 Fast Path 已发布日期属于该周的目标且尚无同周结果时，使用这些公开目标显示“准备抢”，不再限定固定下周。真实事实始终优先，且没有公开目标时不自行伪造未知课程。
- 当前芭蕾页的信息顺序是：顶部概览（训练概览 + 课程卡 + 成长 / 课程等级）→ 课程计划 → 完整本周课程表 → 训练记录。课程计划宽桌面左侧“课程预约”占三分之一，右侧使用独立自然高度容器：顶部“抢课助手 / 本次抢课”严格各半，两块外框与两边各四张内部统计卡均同宽同高且不保留额外底部 margin；下方紧接占满右区的“周安排”，不会再被左侧长预约列表强制拉高或推远。课程预约按完整自然日保留最近三个有课日期，并用课表 Owner 状态补齐当天已经开始、因而退出 upcoming 的课程；只剩当天已预约课表事实且取消字段已退出快照时，页面显示已过可取消时段 / 课程已结束，不伪造截止时间。周安排按日期、时间与标准课型语义去重，真实课程复用完整课表的左侧时间轴、空档压缩、分钟定位和课型色；每个日期再固定拆成“大教室 / 小教室”两个子列，真实课程按教室落位，同教室冲突才分轨，卡内不重复教室名并以紧凑字号保留课程名、老师、时间和状态。压缩空档左侧只显示起始整点，不显示范围或下沿结束整点，时间轴在全周最后一节课的准确结束分钟收口，底部重复课型图例和说明已移除。Fast Path 泛化名称不能与预约快照重复成两节。所选未来周无真实课程时，按日期匹配的公开目标仍显示规则与小字号执行优先级。训练记录固定最后，并继续按本月 / 今年 / 全部与节数 / 时间统一驱动课型、级别、老师和历史。
...
- 服务器私有 canonical ledger 是上课事实唯一机器真相：首次全量回填，日常重扫最近 60 个逻辑日并幂等 upsert，每月 1 日 00:47 全量校验；rolling 任务每天 `09:00 / 12:00 / 15:00 / 18:00 / 22:00` 运行，并在周日 14:30 额外检查抢课后发布的下周课表。周简报专用的周期收尾 timer / service 已停用并删除；同步器不再生成 weeklyBrief 字段，旧缓存中残留字段不再使用。同步失败保留上次成功账本与聚合，只更新独立 `sync-state.json`；预约、课程卡与课表分别保存脱敏快照。
...
- `dash/data/ballet.*` 只能保存脱敏前端读模型与预聚合统计；同步状态同时保存最近尝试、最近成功、数据变更时间、逻辑日期、抓取窗口、连续失败和安全错误码。课程卡支持同页多卡及 `active / expired` 状态，只允许输出名称、有效期、总 / 剩余 / 已用课次和由该卡自身有效期 / 使用量推导的计划与实际节奏；已失效卡缺少闻道总次数时保留 `null` 并继续展示，不推算已用课次，计划预测只作用于字段完整的使用中卡。不同卡不能共享消耗样本，开卡未满 28 天时实际节奏字段保持空值。`PHPSESSID`、Cookie、OAuth code、openid、unionid、memberId、手机号、会员卡号、源记录 ID、原始响应正文和真实执行参数必须只留在服务器隔离运行态，不能进入前端、仓库、日志或聊天。
- 生产凭据已于 2026-07-27 使用新会话重新密封为 host-bound systemd 加密凭据，通过 `LoadCredentialEncrypted` 注入。enable gate 已保留；rolling timer 为每天 `09:00 / 12:00 / 15:00 / 18:00 / 22:00` + 周日 14:30 / 20:00，月度 full 保持每月 1 日 00:47。2026-08-01 10:40 部署新日程时，systemd 因 `Persistent=true` 自动补跑一次已错过的 09:00 rolling，只读同步成功且未执行预约类动作；后续下一次为 12:00。身份失效时采集器保留旧缓存、记录脱敏错误并停止重试，直到检测到非敏感凭据版本变化。
- 2026-07-28 已部署 `maxnow-ballet-live` Skill 与实时查询 CLI：以后 Owner 在对话中询问芭蕾课程数据时，必须 SSH 到 MaxNow 服务器并用当前 PHPSESSID 查询闻道，不再读取前端缓存。服务器 OpenClaw Skill 入口已指向仓库版本；查询只允许既有 GET allowlist 与固定上课历史分页 POST，输出无源记录 ID、会员标识、响应正文或凭据痕迹，实时失败即明确失败。Owner 泛问“有什么课程 / 有什么可以约的课”时默认展示全部课型，只有明确要求“只看 / 仅看 / 只想看”某类课程时才筛选；直约课按日期分组并固定展示开始–结束时间、课程名、老师、教室和余位。最终当天课表验收返回 7 节课，且 Dashboard 缓存、私有账本和临时凭据目录均未留下改动。
...
- 2026-08-09 Owner 用课表箭头明确将 Fast Path 收敛为两节周日大教室课程：17:30–19:00 芭蕾 L1、19:00–20:00 肌肉素质，均不限老师；此前七项目标全部退出配置。生产 fast path 仍于周日 14:19:35 启动、14:20:00 提交，同日课表共享读取，真实 mutation 严格串行，结束后统一实时核验。匹配与 occurrence 幂等键只使用日期、课型 / 等级、起止时间和教室；老师临时变化不会阻止预约或候补。可预约则预约、可排队则候补；身份失效、配置错误或页面结构变化仍全局停止，不取消、转课或支付。
- 2026-08-08 已修复普通预约与 Fast Path 的课程 / 按钮顺序耦合：旧实现复用展示层已排序的课程记录，再与源 HTML 顺序的按钮数组按位置配对；同时间课程的源顺序与展示排序不同时可能提交另一课程按钮。新实现由预约专用解析器在每个 `.classtable` 块内一次解析并绑定课程事实和按钮，Fast Path 复用同一绑定入口，展示排序不再进入预约链；小教室 L1.5 与大教室 L1 的双向顺序回归测试覆盖本次事故。
...
- 2026-08-02 训练详情进一步移除图表与历史之间的空列：图表轨道按热力图 / 折线实际宽度收敛，历史紧接右侧并占满余宽；桌面历史改为两列最多 8 条，`560px` 以下单列最多 3 条。超量时入口位于历史标题区，打开居中、限高且列表内部滚动的“全部记录”悬浮层，不再使用整宽底部按钮或整页高右侧抽屉。图表标题已包含指标，因此不再额外显示孤立的“节 / h”单位徽标。
- 2026-08-02 训练趋势图标题移入左侧完整卡片，并增加 `Training trend` 眉题；左侧趋势卡与右侧历史卡共用边框、圆角、渐变和 hover，桌面同顶同底，内部热力图 / 折线图取消第二层卡片外框。
- 2026-08-31 训练记录“全部”范围不再使用只有单年份时辨识度很低的年度单点折线，改为周一至周日聚合的年度分组热力图；每格显示周起始日期和节数 / 小时，补齐首个训练周到最近成功同步周之间的零课周，并在桌面 / 平板 / 手机按 13 / 7 / 4 列自适应。
...
- 2026-09-20 Owner 停用训练周简报，只保留粉色手作 `week N` 封面。顶部入口仍位于“已同步”右侧，按 `2026-07-27` 至 `2026-08-02 = week 2` 的北京时间周一边界计算；弹窗直接展示单张封面，可复制或下载，空闲时间预生成并只在浏览器缓存。`template.json` 只管理封面 WebP 底图与手绘数字；简报切换、定时生成、专用底图 / 字体及周期收尾任务均已删除。
...
- 2026-07-27 14:23 已在不访问闻道、不读取凭据的前提下向服务器私有 ledger 补入 Owner 确认的 2026-07-25 11:30–12:30 李俊软开课；2026-07-30 20:21 补入 2026-07-30 18:45–19:45 李俊大教室软开课；2026-09-13 10:28 补入当天 10:00–11:30 李俊大教室“芭蕾L1-入门”。第二次补录曾因 root 原子替换把 `attendance-ledger.json` 留成 `root:root 0600`，导致 2026-08-01 00:00 rolling 任务无法以 `ubuntu` 读取并报 `parse_error`。00:39 已恢复 `ubuntu:www-data 0600` 并以服务用户校验，00:40 rolling 只读同步成功；代码与运维规则现要求 root 原子写继承原属主、手工维护后固定用 `ubuntu` 复验，预检失败也要保留旧数据并在顶部明确显示“同步失败”。三条手工记录继续使用 `manual` 稳定键并在 full sync 中保留。
- 这些实验和生产同步都没有提交预约 / 候补 / 取消 / 转课，也不能证明精确静默闲置寿命。Owner 已批准启用每天 `09:00 / 12:00 / 15:00 / 18:00 / 22:00`、周日额外 14:30 / 20:00 rolling 和每月 1 日 00:47 full 只读 timer；周日 rolling 已与 14:20 自动抢课错开。`PHPSESSID` 必须按约课网站密码处理，不得进入 Git、前端、日志、备份、聊天、环境变量或命令参数；删除本地凭据不等于吊销闻道服务端会话。
...
- Home 右侧已接入豆奶签到只读摘要卡片，点击可进入豆奶详情 tab；详情页展示近 30 天实际使用流量、日均可用、签到流量和签到时长折线图。数据来自 `dash/data/dounai_checkin.json`；签到由 Owner 手动完成，服务器 00:05 只读日结负责回读手动签到变更、刷新账号余量并统计真实流量。
...
- 豆奶、Token、芭蕾、云服务、生活和同行记统一使用 `secondary-view` 视觉协议：轻色渐变白底、统一圆角 / 阴影 / hover 与状态 pill；芭蕾顶部栏已有页面名称，因此内容区不再重复标题卡，只保留低权重数据时间与连接状态。芭蕾顶部“本周训练 / 课程卡”在 `1501px` 以上各占内容区三分之一，右侧第三格纵向放置成长等级与课程等级两张独立面板；`1101px–1500px` 前两张各占一半，两个等级面板在下一行上下排列，`1100px` 以下全部单列。课程计划继承同一 `panel` 家族：宽桌面左侧预约占三分之一，右侧顶部助手 / 结果等分、下方周安排占满；`1320px` 以下预约优先整行、助手 / 结果并排、周安排随后，`860px` 以下全部单列。周安排七列在窄屏仅自身横向滚动，整页不得溢出。训练记录固定为最后一个模块；自动抢课完整运行态与 Session 详情继续归 Cloud。2026-07-11 起所有 Dash tab 卡片统一取消顶部彩色横条，语义色只保留在文字、数值、图标、状态点、pill 和轻背景中。
...
4. 闻道 v7 会话生命周期实验正在按每 20 分钟无限期运行；持续观察身份失效、Session 轮换与 `Set-Cookie`。生产每天 `09:00 / 12:00 / 15:00 / 18:00 / 22:00`、周日额外 14:30 / 20:00 rolling 与每月 full timer 已启用；后续观察课表发布和身份阻断，再决定是否推进 personal-wiki 学习记录同步。
