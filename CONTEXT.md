...
- 2026-09-08 已修复豆奶 HTTP 401、空流量行和账号关键字段缺失被误判成功的问题：抓取失败必须保留最后有效数据、标记 `stale / last_error` 且不得推进伪成功时间；登录态可用服务器既有账号密码配合人工图形验证码刷新。当前账号页仍缺少可解析的有效期字段，线上保留 9 月 1 日最后完整账号快照并标旧；真实流量已补齐到 9 月 7 日。豆奶签到页另有独立计算验证码，09:00 自动任务检测到 `checkin_captcha_code` 后必须在 POST 前失败关闭，不能消耗验证次数或伪报签到成功；00:05 只读流量日结不受影响。
