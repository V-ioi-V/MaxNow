...
- 2026-09-08 已修复豆奶 HTTP 401、空流量行和账号关键字段缺失被误判成功的问题：抓取失败必须保留最后有效数据、标记 `stale / last_error` 且不得推进伪成功时间；登录态可用服务器既有账号密码配合人工图形验证码刷新。当前账号页仍缺少可解析的有效期字段，线上保留 9 月 1 日最后完整账号快照并标旧；真实流量已补齐到 9 月 7 日。豆奶签到页另有独立计算验证码，Owner 决定以后手动签到；root crontab 已移除 09:00 签到预检与微信提醒，只保留 00:05 的 `--traffic-only --exclude-today` 只读流量日结。服务器不得再定时提交签到、消耗验证次数或发送签到提醒。
...
- Home 右侧已接入豆奶签到只读摘要卡片，点击可进入豆奶详情 tab；详情页展示近 30 天实际使用流量、日均可用、签到流量和签到时长折线图。数据来自 `dash/data/dounai_checkin.json`；签到由 Owner 手动完成，服务器只保留 00:05 traffic-only cron 统计真实流量。
