...
- 2026-07-26 19:07 已开始第一阶段服务器持续活动实验：临时单元 `maxnow-wenda-session-lifetime-20260726-v3.service` 每 10 分钟只读访问课程表，最长 30 天且服务器重启后不会自动恢复，首条样本为 HTTP 200 / authenticated。探针使用 root 只读代码、systemd `DynamicUser` 与 `LoadCredential`；凭据上传源已删除，活动凭据只存在于 systemd `/run/credentials` 运行态，服务退出后自动消失。Windows 原实验任务已移除，Codex 心跳自动化 `session` 每 30 分钟只读检查脱敏日志。
...
- 豆奶、Token、云服务、生活和同行记统一使用 `secondary-view` / `secondary-page-head` 视觉协议：轻色渐变白底、统一圆角 / 阴影 / hover 与状态 pill；2026-07-11 起所有 Dash tab 卡片统一取消顶部彩色横条，语义色只保留在文字、数值、图标、状态点、pill 和轻背景中。
