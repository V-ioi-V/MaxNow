...
- 2026-09-08 已修复豆奶 HTTP 401 / 空流量行被误判成功的问题：抓取失败必须保留最后有效数据、标记 `stale / last_error` 且不得推进 `updatedAt`；登录态可用服务器既有账号密码配合人工图形验证码刷新。豆奶签到页当前另有独立计算验证码，09:00 自动任务检测到 `checkin_captcha_code` 后必须在 POST 前失败关闭，不能消耗验证次数或伪报签到成功；00:05 只读流量日结不受影响。
