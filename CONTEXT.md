...
- 当前芭蕾页的信息顺序是：顶部概览（训练概览 + 课程卡 + 成长 / 课程等级）→ 课程计划 → 训练记录。课程计划本身两行四区：第一行“课程预约 / 周安排”在宽桌面按三分之一 / 三分之二排布，第二行“抢课助手 / 上次抢课结果”各占一半。周安排按周一至周日切换上周、本周、下周；下周抢课前显示公开目标规则与小字号执行优先级，有同周 `lastRun.records` 后显示真实预约 / 候补 / 失败状态。抢课助手只保留累计抢到、累计候补、上次耗时和运行状态四项；上次结果只展示成功、候补、未抢到总体数量，不再展开逐课明细。训练记录仍固定最后，并继续按本月 / 今年 / 全部与节数 / 时间统一驱动课型、级别、老师和历史。
