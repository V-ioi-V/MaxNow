...
- `dash/data/ballet.json`：芭蕾页面与 Home 摘要读取的脱敏 read model，保存同步状态、累计 / 月度 / 年度聚合、实际上课记录和预约快照，不保存 Cookie、会员标识或原始响应。
...
- 固定 25 分钟阶段已于 2026-07-26 23:28 在首个标准检查返回登录失效并停止；最后认证样本为 23:03，已确认有效时长 3 小时 56 分 06 秒。三阶段结果属于可行性证据，不是最终登录方案，也不能证明精确空闲寿命或滑动续期；unit、原始日志、凭据挂载和停止方式只在 `SERVER_RUNBOOK.md` 维护。生产 00:17 抓取继续待启用，阻塞点改为刷新加密凭据和首次受控验证；Owner 已指定下一次新会话探针改回每 20 分钟，历史 v5 仍按真实 25 分钟展示。
...
- 状态发布器以专用无登录账号运行；已停止实验的日志 inode 固定为 root-owned，并通过同 inode 的只读硬链接映射到 StateDirectory 外的专属源目录，`/var/lib/private` 继续保持 `0700`，不会为了页面读数放宽整个 systemd 私有父目录。
...
- 生产凭据已于 2026-07-26 在服务器内部密封为 host-bound systemd 加密凭据，通过 `LoadCredentialEncrypted` 注入；版本 `1.0.5.00` 的页面、同步器和 unit 已部署，生产 enable gate 与 timer 仍未启用。身份失效时采集器保留旧缓存、记录脱敏错误并停止重试，直到检测到非敏感凭据版本变化；Owner 看到的操作提示固定为在电脑微信重新登录后安全刷新凭据，不展示任何 Session 值。
...
- 2026-07-26 已完成并部署“芭蕾学习模块 + 受控约课”的只读层：导航位于 Token 与云服务之间，页面采用粉玫瑰 + 白卡，机器事实使用首次全量 / 60 日滚动 upsert / 每月全量校验的本地账本，趋势使用月 / 年 / 全部与节数 / 小时切换。目标入口是微信公众号内 H5 而非小程序，通过微信 `snsapi_base` OAuth 建立 `PHPSESSID` 网站会话；一次只读验证已证明，电脑微信退出后，MaxNow 服务器仍可使用该会话访问首页和课程表而不重新 OAuth。主分支 `7a57225` 已部署到服务器，生产 unit 已安装但保持 disabled，enable gate 不存在。
...
- 这项实验没有提交预约 / 候补 / 取消 / 转课，也不能证明精确静默闲置寿命。生产每日 GET 继续保持待启用，直到 Owner 在电脑微信重新登录、安全替换加密凭据并批准首次受控验证；后续约课仍按 `ROADMAP.md` 的 Later 分阶段推进。`PHPSESSID` 必须按约课网站密码处理，不得进入 Git、前端、日志、备份、聊天、环境变量或命令参数；删除本地凭据不等于吊销闻道服务端会话。
...
3. 闻道 Session 生命周期实验已因身份失效结束；下一步是 Owner 在电脑微信重新登录并安全刷新服务器凭据，以每 20 分钟启动新会话探针。只有首次只读验证通过并获得 Owner 单独批准后，才启用芭蕾生产 00:17 同步，并验证首次全量、增量去重、身份失败阻断和旧缓存保留，再推进 personal-wiki 学习记录同步。
