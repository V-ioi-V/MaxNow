...
- `scripts/sync_ballet.py`：只访问已确认的闻道业务只读页面，从服务器私有 canonical ledger 增量去重并刷新脱敏 `dash/data/ballet.*`；同时读取预约、候补、上课记录、课程卡概览和严格日期路径的周课表。上课历史首屏少于累计数时只调用官方脚本声明的固定分页 POST；摘要行必须按日期与课程名唯一命中既有私有账本，不能据此新建或猜测详情。同步不调用预约、取消、候补或转课写接口。
- `scripts/query_ballet_live.py` / `scripts/run_ballet_live_query.sh`：面向对话请求的实时只读入口。通过临时 systemd unit 解密服务器 host-bound PHPSESSID，按课表、当前预约、上课记录或课程卡最小范围直接读取闻道并输出脱敏 JSON；上课记录可使用同一固定分页 POST，但若查询范围包含只有摘要、没有详情 ID 的旧记录则失败关闭。不读取或写入 Dashboard 缓存与服务器私有账本。
...
- `scripts/book_ballet_fast.py` / `server/maxnow-ballet-booking-fast.*`：周日 14:20 无人值守自动抢课关键路径。14:19:35 启动并预热，放课后动态扫描周一至周六，不再依赖每周固定时间表；周一至周五只处理 18:40（含）后开始的课，周六只处理 18:00 前开始的课。所有标准芭蕾 L1、标准芭蕾 L1.5 与课程名严格等于“软开”或“软开课”的课程进入目标，软开专项、软开-胯和周日课程明确排除。课程层为全部 L1、全部 L1.5、全部软开 / 软开课；每层依次为周六、工作日李俊（空老师按李俊）、工作日其他老师，各组内按日期和开始时间排序。六天课表最多 3 路并发读取，课程卡与规则最多 2 路并发预检并设 15 秒有效期，HTTPS 保持最多 3 条 keep-alive 连接，真实 mutation 始终严格串行，最后预约详情并发只读核验。老师不进入 occurrence 幂等键；工作日同一日期、课程名和时间先选李俊老师层，未命中再选其他老师层，每层仍按大教室优先、小教室兜底并要求唯一命中；周六不按老师分层。目标课可预约则预约、仅可排队则候补，已预约 / 已排队不重复提交；可确认未产生 mutation 的临时失败最多重试 3 次，mutation 结果未知不重复提交但不阻止后续课程，全部结束后统一核验 booked / waitlist。每次真实执行把 `attemptedAt + criticalPathMilliseconds` 追加到私有耗时历史，公开状态只发布样本数与平均值；私有幂等状态和公开脱敏状态继续分目录保存。抢课单元每次结束后通过 systemd `OnSuccess` / `OnFailure` 立即触发独立业务只读 rolling 同步，使 `ballet.*` 面板数据跟进最新预约 / 候补；同步只允许 GET 与固定上课历史分页 POST，失败不覆盖抢课单元结果。
