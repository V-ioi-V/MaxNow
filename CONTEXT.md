...
- 当前芭蕾页的信息顺序是：顶部概览（训练概览 + 课程卡 + 成长 / 课程等级）→ 课程计划 → 完整本周课程表 → 训练记录。课程计划本身两行四区：第一行“课程预约 / 周安排”在宽桌面按三分之一 / 三分之二排布，第二行“抢课助手 / 上次抢课结果”各占一半。课程预约按完整自然日保留最近三个有课日期，并用课表 Owner 状态补齐当天已经开始、因而退出 upcoming 的课程；周安排按日期、时间与标准课型语义去重，真实课程复用完整课表卡片样式，Fast Path 泛化名称不能与预约快照重复成两节。下周抢课前仍显示公开目标规则与小字号执行优先级。训练记录固定最后，并继续按本月 / 今年 / 全部与节数 / 时间统一驱动课型、级别、老师和历史。
...
- 豆奶、Token、芭蕾、云服务、生活和同行记统一使用 `secondary-view` 视觉协议：轻色渐变白底、统一圆角 / 阴影 / hover 与状态 pill；芭蕾顶部栏已有页面名称，因此内容区不再重复标题卡，只保留低权重数据时间与连接状态。芭蕾顶部“本周训练 / 课程卡”在 `1501px` 以上各占内容区三分之一，右侧第三格纵向放置成长等级与课程等级两张独立面板；`1101px–1500px` 前两张各占一半，两个等级面板在下一行上下排列，`1100px` 以下全部单列。课程计划继承同一 `panel` 家族，宽桌面顶部预约 / 周安排为 1:2、底部助手 / 结果为 1:1；`1320px` 以下先堆叠顶部两区，`860px` 以下全部单列。周安排七列在窄屏仅自身横向滚动，整页不得溢出。训练记录固定为最后一个模块；自动抢课完整运行态与 Session 详情继续归 Cloud。2026-07-11 起所有 Dash tab 卡片统一取消顶部彩色横条，语义色只保留在文字、数值、图标、状态点、pill 和轻背景中。
