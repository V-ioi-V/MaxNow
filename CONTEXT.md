...
- `scripts/sync_token_usage.py`：合并 OpenClaw / Codex local / Codex server 源账本，生成 Token 页面优先读取的统一总账。
...
- 本机 Codex 用量可以由 `python scripts/update_data.py codex-usage` 刷新；服务器 Codex 用量由 root 运行 `python3 scripts/update_data.py codex-server-usage` 刷新。两者都只读取 `.codex/sessions` 中的 `token_count` 与 `turn_context.model`，只导出 token 统计、模型名、时间戳、来源和 OpenAI API 等价费用估算，不导出对话正文。
...
- `dash/data/token-usage.json` 是 Token 页统一入口；OpenClaw、Codex local、Codex server 和后续其他来源都应合入这个总账。Token 页来源列表显式展示这三类来源。
...
3. 给 Codex Token 统计继续补服务器侧 collector / cron、失败日志和部署目录权限；本机 Windows Task Scheduler 上报已落地。
