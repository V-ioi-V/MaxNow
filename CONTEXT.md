...
- 2026-08-02 Owner 确认芭蕾每周小红书封面采用固定粉色手作系列。页面已用 `2026-07-27` 至 `2026-08-02 = week 2` 的北京时间周一边界计算周数；顶部 `week N` 入口点击后在浏览器本地拼合版本化底图与透明手绘 `0–9`，支持预览、下载和剪贴板复制，不每周调用 AI、不上传图片、不需要服务器定时任务。`dash/assets/ballet-week-cover/template.json` 是底图版本、文件名和数字位置的唯一配置；Owner 更换无数字底图时使用新文件名并提升 `templateVersion`，页面脚本无需修改。
