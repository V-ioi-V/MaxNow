...
- 当前芭蕾页的信息顺序是：顶部概览（训练概览 + 课程卡 + 成长 / 课程等级）→ 课程计划 → 完整本周课程表 → 训练记录。课程计划宽桌面左侧“课程预约”占三分之一，右侧使用独立自然高度容器：顶部并排“抢课助手 / 本地抢课结果”，两块外框和内部统计卡同高且不保留额外底部 margin；下方紧接占满右区的“周安排”，不会再被左侧长预约列表强制拉高或推远。课程预约按完整自然日保留最近三个有课日期，并用课表 Owner 状态补齐当天已经开始、因而退出 upcoming 的课程；只剩当天已预约课表事实且取消字段已退出快照时，页面显示已过可取消时段 / 课程已结束，不伪造截止时间。周安排按日期、时间与标准课型语义去重，真实课程复用完整课表的左侧时间轴、空档压缩、分钟定位、课型色和老师 / 教室 / 时间 / 状态结构；压缩空档左侧只显示起始整点，不显示范围或下沿结束整点，时间轴在全周最后一节课的准确结束分钟收口，底部重复课型图例和说明已移除。Fast Path 泛化名称不能与预约快照重复成两节。下周抢课前仍显示公开目标规则与小字号执行优先级。训练记录固定最后，并继续按本月 / 今年 / 全部与节数 / 时间统一驱动课型、级别、老师和历史。
