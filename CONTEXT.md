...
- `dash/data/ballet-booking-fast.json`：芭蕾“抢课”工作区与 Cloud 自动抢课运维卡共用的安全 fallback；生产状态由周日 fast-path service 写入独立 public 目录，经登录校验和 `no-store` exact alias 提供。芭蕾页用它渲染累计已抢到、上次执行关键路径总耗时与逐目标平均值、脱敏代抢目标和独立结果 Tab；“已预约”仍取 `ballet.json` 当前未来预约 / 候补列表。Cloud 读取启用状态、优先级、上次 / 下次执行和累计数；该数据不作为课程余位事实源。
...
- 课表课程卡始终保留课程名后的独立老师行，包括中等桌面、60 分钟紧凑卡和重叠窄卡；人数 / 排队与时间 / 状态各自压缩为单行，不再通过隐藏老师换取空间。这个显示规则与自动抢课是否限制老师相互独立。
...
- 2026-08-02 本月训练热力图改为内容驱动的紧凑卡：桌面最大 `840px` 并左对齐，手机端继续占满可用宽度；格子高度与间距收紧，未来尚未同步的日期只保留低对比浅底虚线格，不再用密集斜纹和重复破折号占据视觉焦点。
