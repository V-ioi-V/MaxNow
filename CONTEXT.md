...
- 芭蕾成长评分的唯一人类可读协议放在 `SPEC.md` 的“成长评分协议”，课程指南提供课程路径与阶段语义，MaxNow 自己维护评分和 XP 的游戏化参数。任何升班课次 / 月份 / 70:30 权重 / 规律判定 / XP 权重 / 等级阈值 / 名称 / 效果变更，都必须在同一分支同步修改 `dash/app.js`、`SPEC.md` 和 `UPDATE_LOG.md`；`scripts/check.py` 对代码常量与规范表格做一致性校验。
